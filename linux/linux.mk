...
else ifeq ($(BR2_LINUX_KERNEL_CUSTOM_ZIP),y)
# Handle custom zip as specified by the configuration
LINUX_ZIP = $(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_ZIP_LOCATION))
LINUX_SITE = $(patsubst %/,%,$(dir $(LINUX_ZIP)))
LINUX_SOURCE = $(notdir $(LINUX_ZIP))
define LINUX_EXTRACT_CMDS
        unzip $(LINUX_DL_DIR)/$(LINUX_SOURCE) -d $(@D)
        mv $(@D)/`ls $(@D)`/* $(@D)
endef
