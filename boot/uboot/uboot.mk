...
else ifeq ($(BR2_TARGET_UBOOT_CUSTOM_ZIP),y)
# Handle custom U-Boot zip as specified by the configuration
UBOOT_TARBALL = $(call qstrip,$(BR2_TARGET_UBOOT_CUSTOM_ZIP_LOCATION))
UBOOT_SITE = $(patsubst %/,%,$(dir $(UBOOT_TARBALL)))
UBOOT_SOURCE = $(notdir $(UBOOT_TARBALL))
define UBOOT_EXTRACT_CMDS
        unzip $(UBOOT_DL_DIR)/$(UBOOT_SOURCE) -d $(@D)
        mv $(@D)/`ls $(@D)`/* $(@D)
endef
