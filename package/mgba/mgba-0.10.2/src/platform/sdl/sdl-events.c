...
static struct SDL_JoystickCombo* _mSDLOpenJoystick(struct mSDLEvents* events, int i) {
	...
	if (joystick->controller) {
		if (SDL_GameControllerHasSensor(joystick->controller, SDL_SENSOR_GYRO) &&
		    !SDL_GameControllerIsSensorEnabled(joystick->controller, SDL_SENSOR_GYRO)) {
			SDL_GameControllerSetSensorEnabled(joystick->controller, SDL_SENSOR_GYRO, SDL_TRUE);
		}
		if (SDL_GameControllerHasSensor(joystick->controller, SDL_SENSOR_ACCEL) &&
...
void mSDLInitBindingsGBA(struct mInputMap* inputMap) {
...
#else
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_z, GBA_KEY_A);
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_x, GBA_KEY_B);
	...
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_RSHIFT, GBA_KEY_SELECT);
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_UP, GBA_KEY_UP);
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_DOWN, GBA_KEY_DOWN);
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_LEFT, GBA_KEY_LEFT);
	mInputBindKey(inputMap, SDL_BINDING_KEY, SDLK_RIGHT, GBA_KEY_RIGHT);
...
bool mSDLAttachPlayer(struct mSDLEvents* events, struct mSDLPlayer* player) {
	...
	for (i = 0; i < SDL_JoystickListSize(&events->joysticks); ++i) {
		...
		SDL_JoystickGetGUIDString(SDL_JoystickGetGUID(SDL_JoystickListGetPointer(&events->joysticks, i)->joystick),
...
#endif
		if (events->preferredJoysticks[player->playerId] &&
		    strcmp(events->preferredJoysticks[player->playerId], joystickName) == 0) {
			index = i;
			break;
		}
	}
	...
	while (SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_JOYDEVICEADDED, SDL_JOYDEVICEREMOVED) > 0) {
		if (event.type == SDL_JOYDEVICEADDED) {
			...
			for (i = 0; i < events->playersAttached && i < MAX_PLAYERS; ++i) {
				joysticks[i] = events->players[i]->joystick ?
...
static void _mSDLHandleKeypress(struct mCoreThread* context, struct mSDLPlayer* sdlContext,
	...
	if (event->type == SDL_KEYDOWN) {
		...
		default:
			if ((event->keysym.mod & GUI_MOD) && (event->keysym.mod & GUI_MOD) == event->keysym.mod) {
				...
				case SDLK_f:
					SDL_SetWindowFullscreen(sdlContext->window,
					                        sdlContext->fullscreen ? 0 : SDL_WINDOW_FULLSCREEN_DESKTOP);
			...
			if (event->keysym.mod & KMOD_SHIFT) {
				...
				case SDLK_F9:
					mCoreThreadInterrupt(context);
					mCoreSaveState(context->core, event->keysym.sym - SDLK_F1 + 1,
					               SAVESTATE_SAVEDATA | SAVESTATE_SCREENSHOT | SAVESTATE_RTC);
					mCoreThreadContinue(context);
					break;
				default:
					break;
				...
				case SDLK_F9:
					mCoreThreadInterrupt(context);
					mCoreLoadState(context->core, event->keysym.sym - SDLK_F1 + 1,
					               SAVESTATE_SCREENSHOT | SAVESTATE_RTC);
					mCoreThreadContinue(context);
					break;
				default:
					break;
...
static void _mSDLHandleJoyButton(struct mCoreThread* context, struct mSDLPlayer* sdlContext,
                                 const struct SDL_JoyButtonEvent* event) {
	int key = 0;
...
static void _mSDLHandleJoyHat(struct mCoreThread* context, struct mSDLPlayer* sdlContext,
                              const struct SDL_JoyHatEvent* event) {
...
static void _mSDLHandleJoyAxis(struct mCoreThread* context, struct mSDLPlayer* sdlContext,
                               const struct SDL_JoyAxisEvent* event) {
