...
bool GBAIsMB(struct VFile* vf) {
	...
	for (i = 0; i < 128; ++i) {
		...
		if ((info.operandFormat & ARM_OPERAND_MEMORY) && info.memory.baseReg == ARM_PC &&
...
#ifdef USE_DEBUGGERS
	if (gba->debugger) {
		struct mDebuggerEntryInfo info = { .address = _ARMPCAddress(cpu), .type.bp.opcode = opcode };
...
void GBABreakpoint(struct ARMCore* cpu, int immediate) {
	...
	case CPU_COMPONENT_DEBUGGER:
		if (gba->debugger) {
			struct mDebuggerEntryInfo info = { .address = _ARMPCAddress(cpu),
...
#ifdef USE_DEBUGGERS
static bool _setSoftwareBreakpoint(struct ARMDebugger* debugger, uint32_t address, enum ExecutionMode mode,
                                   uint32_t* opcode) {
...
static void _clearSoftwareBreakpoint(struct ARMDebugger* debugger, const struct ARMDebugBreakpoint* breakpoint) {
	GBAClearBreakpoint((struct GBA*) debugger->cpu->master, breakpoint->d.address, breakpoint->sw.mode,
	                   breakpoint->sw.opcode);
}
#endif
