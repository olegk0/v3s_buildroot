...
bool Game::main_loop()
{
	...
	while ((m_level->is_player_alive()) || ((!input->get_fire()) && (!input->get_enter()) && (!input->get_die()))) {
		if (input->get_pause()) {
			SDL_Delay(200);
			input->update();
	...
	while (play != Menu::MENU_EPIPHANY_QUIT) {
		if (play == Menu::MENU_EPIPHANY_START) {
			...
			sprintf(current_level_path, "%s/maps/level%d.map",
				Resource_Factory::instance()->get_resource_path().c_str(), menu.get_current_level());
...
void Game::init()
{
	...
	Screen::instance()->init(m_config->get_screen_size_x(), m_config->get_screen_size_y(),
				 m_config->get_map_size_x() * k_sprite_size, m_config->get_map_size_y() * k_sprite_size);
