...
{
	return m_screen; // m_virtual_screen;
}

SDL_Surface *Screen::convert_surface_format(SDL_Surface *surface)
{
	//SDL_Surface *temp = SDL_CreateRGBSurface(0, surface->w, surface->h, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
	SDL_Surface *temp = SDL_CreateRGBSurface(0, surface->w, surface->h, 32, m_screen->format->Rmask, m_screen->format->Gmask,
...
void Screen::init(Uint32 resolution_x, Uint32 resolution_y, Uint32 world_size_x, Uint32 world_size_y)
{
	...
	/*
		m_window = SDL_CreateWindow("My Game Window",
                          ...
                          SDL_WINDOW_SHOWN);
*/
	m_screen = SDL_SetVideoMode(resolution_x, resolution_y, 32, SDL_HWSURFACE | SDL_DOUBLEBUF);
	if (m_screen == NULL) {
		printf("Unable to set %d x %d video mode: %s\n", resolution_x, resolution_y, SDL_GetError());
		exit(1);
	}
	//SDL_SetAlpha(m_screen, SDL_SRCALPHA | SDL_RLEACCEL, 255);
	//m_renderer = SDL_CreateRenderer(m_window, -1, 0);

	//m_virtual_screen = SDL_CreateRGBSurface(0, resolution_x, resolution_y, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
	//m_screen = SDL_CreateTexture(m_renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, resolution_x, resolution_y);
}
...
void Screen::resize_world_screen(Uint32 size_x, Uint32 size_y)
{
	...
	if (size_x != m_world_size_x || size_y != m_world_size_y) {
		//m_virtual_screen = SDL_CreateRGBSurface(0, size_x, size_y, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
	}
...
void Screen::clear()
{
	SDL_Surface *dest_surf = m_screen; //m_virtual_screen;
...
void Screen::flip_display()
{
	...
	//SDL_BlitSurface(m_virtual_screen, NULL, m_screen, NULL);
	/*
	SDL_UpdateTexture(m_screen, NULL, m_virtual_screen->pixels, m_virtual_screen->pitch); m_screen <- m_virtual_screen
	SDL_RenderClear(m_renderer);//This function clears the entire rendering target,
	SDL_RenderCopy(m_renderer, m_screen, NULL, NULL); m_renderer <- m_screen
	SDL_RenderPresent(m_renderer); Update the screen with any rendering performed
	*/
...
void Screen::get_surface(SDL_Surface *surface, SDL_Rect *src, ScreenCoord dest)
{
	...
	rect_dest.x = dest.x;
	rect_dest.y = dest.y;
	SDL_Surface *src_surf = m_screen; // m_virtual_screen;
