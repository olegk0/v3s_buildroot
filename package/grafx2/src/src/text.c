...
#define EXTID4(a, b, c, d)                                                     \
  ((((a)&255) << 24) | (((b)&255) << 16) | (((c)&255) << 8) | (((d)&255)))

static int
Compare_fonts(T_Font* font_1, T_Font* font_2)
{
...
Insert_font(T_Font* font)
{
  ...
  if (font_list_start == NULL) {
    ...
    if (compare <= 0) {
      ...
      while (searched_font->Next &&
             (compare = Compare_fonts(font, searched_font->Next)) > 0)
...
static void
Add_font(const char* name, const char* font_name)
{
  T_Font* font;
  ...
  CFSFontName = CFStringCreateWithBytes(
    NULL, (UInt8*)name, size - 1, kCFStringEncodingASCII, false);
  ...
  switch (EXTID(tolower(name[size - 4]),
                tolower(name[size - 3]),
                tolower(name[size - 2]))) {
    case EXTID('t', 't', 'f'):
    case EXTID('f', 'o', 'n'):
  ...
  for (index = 0;
       index < 17 && font_name[index] != '\0' && font_name[index] != '.';
       index++)
...
#if defined(WIN32) && defined(NOTTF)
static int CALLBACK
EnumFontFamCallback(CONST LOGFONTA* lpelf,
                    ...
                    LPARAM lParam)
{
  ...
  font->Is_truetype = 1;
  snprintf(
    font->Label, sizeof(font->Label), "%-17.17s%s", lpelf->lfFaceName, type);
...
#ifndef NOTTF
byte*
Render_text_TTF(const char* str,
                int font_number,
                int size,
                int antialias,
                int bold,
...
// Text rendering: creates a 8bit surface with its dedicated palette
  ...
  if (antialias) {
    int black_col;
    // Shaded text: X-Swap the color that is pure black with the BG color
    // number, so that the brush is immediately 'transparent'

    // Find black (c)
    for (black_col = 0; black_col < 256; black_col++) {
      if (palette[black_col].R == 0 && palette[black_col].G == 0 &&
          palette[black_col].B == 0)
        break;
    ...
    // Exception: if BG==FG, FG will be set to black or white - any different
    // color.
    long index;
    byte new_fore = Fore_color;

    if (Fore_color == Back_color) {
      new_fore = Best_color_perceptual_except(Main.palette[Back_color].R,
                                              Main.palette[Back_color].G,
                                              Main.palette[Back_color].B,
                                              Back_color);
    }
...
#if defined(WIN32)
byte*
Render_text_Win32(const char* str,
                  int font_number,
                  int size,
                  int antialias,
                  int bold,
                  ...
                  T_Palette palette)
{
  ...
  bi->bmiHeader.biBitCount =
    antialias ? 32 : 8; // the GDI does Antialiasing only on TrueColor DIB
  ...
  rect.bottom = s.cy;
  DrawTextA(
    dc, str, str_len, &rect, DT_LEFT | DT_SINGLELINE | DT_NOCLIP | DT_NOPREFIX);
  GdiFlush();
...
byte*
Render_text_SFont(const char* str,
                  int font_number,
                  int* width,
                  int* height,
                  T_Palette palette)
{
  ...
  *width = SFont_TextWidth(font, str);
  ...
  SFont_FreeFont(font);
  free(
    text_surface); // Do not call Free_GFX2_Surface() because pixels was stolen
            ...
            T_Palette palette)
{
  ...
  if (font->Is_truetype) {
#if !defined(NOTTF)
    return Render_text_TTF(
      str, font_number, size, antialias, bold, italic, width, height, palette);
#elif defined(WIN32)
    return Render_text_Win32(
      str, font_number, size, antialias, bold, italic, width, height, palette);
#else
    return NULL;
#endif
