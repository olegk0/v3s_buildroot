...
Button_Message_initial(void)
{
  ...
  Print_in_window(
    130 - 4 * 26, 88, "Copyright (c) 2007-2023 by", MC_Dark, MC_Light);
  Print_in_window(
    130 - 4 * 23, 100, "the Grafx2 project team", MC_Black, MC_Light);
  Print_in_window(
    130 - 4 * 26, 112, "Copyright (c) 1996-2001 by", MC_Dark, MC_Light);
  ...
  GFX2_Log(
    GFX2_DEBUG, "Button_Message_initial() clicked_button=%d\n", clicked_button);
  Close_window();
...
void
Change_paintbrush_shape(byte shape)
{
  ...
  switch (Current_operation) {
    ...
    case OPERATION_COLORPICK:
      ...
      break;
      // Note: Il existe un Paintbrush_shape_before_lasso, mais comme le lasso
      // aura été automatiquement désactivé avant d'arriver ici, y'a pas de
      // problème.
  }
}
...
void
Button_Pal_right(int btn)
{
  short cells;
  cells = (Config.Palette_vertical) ? Palette_cells_X() : Palette_cells_Y();

  Hide_cursor();
  if ((int)First_color_in_palette + Palette_cells_X() * Palette_cells_Y() <
      256) {
    First_color_in_palette += cells;
    Display_menu_palette();
  }
    ...
    if ((int)First_color_in_palette + (cells_y)*cells_x * 2 < 256)
      ...
      if (Config.Palette_vertical)
        First_color_in_palette =
          255 / cells_x * cells_x - (cells_y - 1) * cells_x;
      else
        First_color_in_palette =
          255 / cells_y * cells_y - (cells_x - 1) * cells_y;
    }
    Display_menu_palette();
  }
  Unselect_button(btn);
...
void
Button_Select_forecolor(int btn)
{
...
void
Button_Select_backcolor(int btn)
{
  int color;
...
Button_Toggle_toolbar(int btn)
{
  ...
  item = Dropdown_activate(
    &dropdown, 0, Menu_Y + Menu_bars[MENUBAR_STATUS].Top * Menu_factor_Y);
  ...
  if (item) {
    switch (item->Number) {
      ...
      case 1: // layers
        ...
        Set_bar_visibility(
          MENUBAR_LAYERS, !Menu_bars[MENUBAR_LAYERS].Visible, 0);
      ...
      case 2: // anim
        ...
        Set_bar_visibility(
          MENUBAR_ANIMATION, !Menu_bars[MENUBAR_ANIMATION].Visible, 0);
...
void
Button_Toggle_all_toolbars(int btn)
{
...
byte
Button_Quit_local_function(void)
{
  short clicked_button;
  ...
  switch (clicked_button) {
    ...
    case 2:     // Sauver et enregistrer
      filename = Filepath_append_to_dir(Main.backups->Pages->File_directory,
                                        Main.backups->Pages->Filename);
      ...
      if (!exists || Confirmation_box("Erase old file ?")) {
        ...
        Init_context_layered_image(&save_context,
                                   Main.backups->Pages->Filename,
                                   Main.backups->Pages->File_directory);
...
//---------------------------- Effacer l'écran -------------------------------
void
Button_Clear(int btn)
{
  Hide_cursor();
  Backup();
  if (Stencil_mode && Config.Clear_with_stencil)
    Clear_current_image_with_stencil(Main.backups->Pages->Transparent_color,
                                     Stencil);
  else
...
void
Button_Clear_with_backcolor(int btn)
{
  Hide_cursor();
  Backup();
  if (Stencil_mode && Config.Clear_with_stencil)
...
const T_Lookup Lookup_MenuRatio[] = {
  ...
  { "x3", 253 },                    // -3
  { "x4", 252 },                    // -4
  { "Moderate", 2 }, { "Maximum", 1 }, { NULL, -1 },
};

const T_Lookup Lookup_MouseSpeed[] = {
  { "Normal", 1 }, { "/2", 2 }, { "/3", 3 }, { "/4", 4 }, { NULL, -1 },
};
...
long int
Get_setting_value(const T_Setting* item)
{
  switch (item->Type) {
    case 1:
...
void
Set_setting_value(const T_Setting* item, long int value)
{
  switch (item->Type) {
    case 1:
...
const char*
Lookup_code(int code, const T_Lookup* lookup)
{
  int i;
...
int
Lookup_next(int code, const T_Lookup* lookup)
{
  int i;
...
int
Lookup_previous(int code, const T_Lookup* lookup)
{
  int count;
  int current = -1;
...
void
Settings_display_config(const T_Setting* setting,
                        T_Config* conf,
                        T_Special_button* panel)
{
  ...
  // A single button
  Print_in_window(
    155, 166, (conf->Auto_save) ? "YES" : " NO", MC_Black, MC_Light);

  // Clear all
  Window_rectangle(
    panel->Pos_X, panel->Pos_Y, panel->Width, panel->Height + 1, MC_Light);
  for (i = 0; i < SETTING_PER_PAGE; i++) {
    Print_in_window(panel->Pos_X + 3,
                    panel->Pos_Y + i * SETTING_HEIGHT +
                      (SETTING_HEIGHT - 6) / 2,
                    setting[i].Label,
                    i == 0 ? MC_White : MC_Dark,
                    MC_Light);
    if (setting[i].Value) {
      ...
      if (setting[i].Lookup) {
        ...
        Print_in_window(panel->Pos_X + 3 + 176,
                        panel->Pos_Y + i * SETTING_HEIGHT +
                          (SETTING_HEIGHT - 6) / 2,
                        str,
                        MC_Black,
  ...
  Update_window_area(
    panel->Pos_X, panel->Pos_Y, panel->Width, panel->Height + 1);
}

void
Settings_save_config(T_Config* conf)
{
  if (Save_CFG())
    Error(0);
  else if (Save_INI(conf))
    Error(0);
}

void
Settings_load_config(T_Config* conf)
{
  if (Load_CFG(0))
    Error(0);
  else if (Load_INI(conf))
    Error(0);
}

void
Button_Settings(int btn)
{
  short clicked_button;
  ...
  const T_Setting setting[SETTING_PER_PAGE * SETTING_PAGES] = {
    ...
    { "Opening message:",
      1,
      &(selected_config.Opening_message),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "Draw limits:",
      1,
      &(selected_config.Display_image_limits),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "Separate colors:",
      1,
      &(selected_config.Separate_colors),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "  on left click:",
      1,
      &(selected_config.Delay_left_click_on_slider),
      1,
      255,
      4,
      NULL },
    { "  on right click:",
      1,
      &(selected_config.Delay_right_click_on_slider),
      1,
      255,
      4,
      NULL },
    { "Merge movement:",
      1,
      &(selected_config.Mouse_merge_movement),
      0,
      100,
      4,
      NULL },
    { "Double click speed:",
      2,
      &(selected_config.Double_click_speed),
      1,
      1999,
      4,
      NULL },
    { "Double key speed:",
      2,
      &(selected_config.Double_key_speed),
      1,
      1999,
      4,
      NULL },
    ...
    { "Virtual keyboard",
      1,
      &(selected_config.Use_virtual_keyboard),
      0,
      2,
      0,
    ...
    { "Adjust brush pick:",
      1,
      &(selected_config.Adjust_brush_pick),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "Vertices per polygon:",
      4,
      &(selected_config.Nb_max_vertices_per_polygon),
      2,
      16384,
      5,
      NULL },
    ...
    { "Clear with stencil:",
      1,
      &(selected_config.Clear_with_stencil),
      0,
      1,
      0,
      Lookup_YesNo },
    { "Auto discontinuous:",
      1,
      &(selected_config.Auto_discontinuous),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "Right click colorpick:",
      1,
      &(selected_config.Right_click_colorpick),
      0,
      1,
      0,
      Lookup_YesNo },
    { "Multi shortcuts:",
      1,
      &(selected_config.Allow_multi_shortcuts),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "  Hidden files:",
      4,
      &(selected_config.Show_hidden_files),
      0,
      1,
      0,
      Lookup_YesNo },
    { "  Hidden dirs:",
      4,
      &(selected_config.Show_hidden_directories),
      0,
      1,
      0,
      Lookup_YesNo },
    ...
    { "Maximize preview:",
      1,
      &(selected_config.Maximize_preview),
      0,
      1,
      0,
      Lookup_YesNo },
    { "Find file fast:",
      1,
      &(selected_config.Find_file_fast),
      0,
      2,
      0,
      Lookup_FFF },
    ...
    { "  According to:",
      1,
      &(selected_config.Set_resolution_according_to),
      1,
      2,
      0,
      Lookup_AutoRes },
    ...
    { "Screen size in GIF:",
      1,
      &(selected_config.Screen_size_in_GIF),
      0,
      1,
      0,
      Lookup_YesNo },
  ...
  const char* help_section[SETTING_PAGES] = {
    "GUI", "INPUT", "EDITING", "FILE SELECTOR", "FILE FORMAT OPTIONS",
  };
  ...
  // Button Reload
  Window_set_normal_button(
    6, 163, 51, 14, "Reload", 1, 1, KEY_r); // 1
                                            // Button Auto-save
  Window_set_normal_button(
    73, 163, 107, 14, "Auto-save:   ", 1, 1, KEY_a); // 2
                                                     // Button Save
  Window_set_normal_button(
    183, 163, 51, 14, "Save", 1, 1, KEY_s); // 3
                                            // Button Close
  Window_set_normal_button(250, 163, 51, 14, "Close", 0, 1, KEY_ESC); // 4

  panel = Window_set_special_button(
    10, 21, 272, SETTING_PER_PAGE * SETTING_HEIGHT, 0); // 5
  Window_set_scroller_button(285,
                             21,
                             SETTING_PER_PAGE * SETTING_HEIGHT,
                             SETTING_PAGES,
                             1,
                             current_page); // 6
  ...
  do {
    if (need_redraw) {
      ...
      Settings_display_config(
        setting + current_page * SETTING_PER_PAGE, &selected_config, panel);
      if (need_redraw & 2) {
    ...
    switch (clicked_button) {
      ...
      case 5: // Panel area
      {
        ...
        int num =
          (((short)Mouse_Y - Window_pos_Y) / Menu_factor_Y - panel->Pos_Y) /
          SETTING_HEIGHT;
        if (num >= 0 && num < SETTING_PER_PAGE) {
          const T_Setting* item =
            &setting[current_page * SETTING_PER_PAGE + num];
          if (item->Type != 0) {
            // Remember which button is clicked
            byte old_mouse_k = Mouse_K;

            if (Window_normal_button_onclick(panel->Pos_X,
                                             panel->Pos_Y +
                                               num * SETTING_HEIGHT,
                                             panel->Width,
                                             SETTING_HEIGHT + 1,
              ...
              if (item->Lookup) {
                ...
                if (Readline(panel->Pos_X + 3 + 176,
                             panel->Pos_Y + num * SETTING_HEIGHT +
                               (SETTING_HEIGHT - 6) / 2,
                             str,
                             item->Digits + 1,
                             INPUT_TYPE_INTEGER)) {
...
//
char*
Format_font_filename(const char* fname)
{
...
static void
Add_font_or_skin(const char* full_name, const char* fname)
{
  size_t namelength;
  (void)full_name;

  namelength = strlen(fname);
  if (namelength >= 10 && fname[0] != '_' &&
      !strncasecmp(fname, SKIN_PREFIX, strlen(SKIN_PREFIX)) &&
      (!strcasecmp(fname + namelength - 4, ".png") ||
       !strcasecmp(fname + namelength - 4, ".gif"))) {
    Add_element_to_list(&Skin_files_list,
                        fname,
                        Format_filename(fname, 19, 0),
                        FSOBJECT_FILE,
                        ICON_NONE);
    ...
    if (fname[0] == '\0')
      return;
             ...
             !strncasecmp(fname, FONT_PREFIX, strlen(FONT_PREFIX)) &&
             (!strcasecmp(fname + namelength - 4, ".png"))) {
    Add_element_to_list(&Font_files_list,
                        fname,
                        Format_font_filename(fname),
                        FSOBJECT_FILE,
                        ICON_NONE);
...
void
Draw_one_skin_name(word x, word y, word index, byte highlighted)
{
...
#define FILESEL_Y 34
  ...
  skin_list = Window_set_list_button(
    ...
    Window_set_special_button(
      8, FILESEL_Y + 1, 144, 80, 0), // 2
                                     // Scroller for the fileselector
    (file_scroller = Window_set_scroller_button(
       155, FILESEL_Y - 1, 82, Skin_files_list.Nb_elements, 10, 0)), // 3
    Draw_one_skin_name,
    2); // 4

  skin_list->Cursor_position =
    Find_file_in_fileselector(&Skin_files_list, Config.Skin_file);
  if (skin_list->Cursor_position < 0)
    skin_list->Cursor_position = 0;

  // Buttons to choose a font
  font_dropdown = Window_set_dropdown_button(
    172,
    43,
    104,
    11,
    0,
    Get_item_by_index(&Font_files_list, selected_font)->Short_name,
    1,
    0,
    1,
  ...
  for (temp = 0; temp < Font_files_list.Nb_files; temp++)
    Window_dropdown_add_item(
      font_dropdown,
      temp,
      Get_item_by_index(&Font_files_list, temp)->Short_name);

  // Cancel
  Window_set_normal_button(61, 120, 51, 14, "Cancel", 0, 1, KEY_ESCAPE); // 6

  // Dropdown list to choose cursor type
  cursor_dropdown = Window_set_dropdown_button(172,
                                               69,
                                               104,
                                               11,
                                               0,
                                               cursors[selected_cursor],
                                               1,
                                               0,
                                               1,
                                               RIGHT_SIDE | LEFT_SIDE,
  ...
  Window_set_normal_button(172,
                           87,
                           14,
                           14,
                           (Config.Display_image_limits) ? "X" : " ",
                           -1,
                           1,
                           KEY_NONE); // 8
  ...
  do {
    if (need_load) {
      ...
      free(skinsdir);
      skinsdir = strdup(
        Get_item_by_index(&Skin_files_list,
                          skin_list->List_start + skin_list->Cursor_position)
          ->Full_name);
    ...
    switch (clicked_button) {
      ...
      case 5: // Font dropdown
      {
        ...
        if (new_font) {
          ...
          Menu_font = new_font;
          Print_in_window(172, 33, "Font:", MC_Black, MC_Light);
          Print_in_window_limited(font_dropdown->Pos_X + 2,
                                  font_dropdown->Pos_Y +
                                    (font_dropdown->Height - 7) / 2,
                                  fontName->Short_name,
                                  (byte)strlen(fontName->Short_name),
                                  MC_Black,
                                  MC_Light);
      ...
      case 9: // Separate colors
        ...
        Hide_cursor();
        Print_in_window(
          175, 114, (separatecolors) ? "X" : " ", MC_Black, MC_Light);
        Display_cursor();
        break;
    }
  } while ((clicked_button != 1) && (clicked_button != 6) &&
           (Key != KEY_ESCAPE) && !Quit_is_required);
  ...
  if (clicked_button == 1) {
    ...
    new_font = Load_font(
      Get_item_by_index(&Font_files_list, selected_font)->Full_name, 1);
    if (new_font) {
      const char* fname;
  ...
  // We don't want to keep the skin's palette, as this would corrupt the current
  // picture's one.
  Set_palette(Main.palette);
...
Button_Page(int btn)
{
  ...
  Pixel_preview =
    (Main.magnifier_mode) ? Pixel_preview_magnifier : Pixel_preview_normal;
  ...
  for (factor_index = 0; ZOOM_FACTOR[factor_index] != Main.magnifier_factor;
       factor_index++)
    ;
    ...
    for (i = 0; i < Spare.backups->Pages->Nb_layers; i++) {
      if (i == Spare.current_layer) {
        ...
        Copy_part_of_image_to_another(
          ...
          0,
          0,
          Min(old_width, Spare.image_width),
          Min(old_height, Spare.image_height),
          old_width,
          Spare.backups->Pages->Image[i].Pixels,
          0,
          0,
          Spare.image_width);
      }
...
Copy_some_colors(void)
{
  ...
  Menu_tag_colors(
    "Tag colors to copy", mask_color_to_copy, &confirmation, 0, NULL, 0xFFFF);
  ...
  if (confirmation) {
    ...
    for (index = 0; index < 256; index++) {
      if (mask_color_to_copy[index])
        memcpy(
          Spare.palette + index, Main.palette + index, sizeof(T_Components));
    }
  }
}

void
Button_Copy_page(int btn)
{
  short clicked_button;
  ...
  Window_set_normal_button(
    10, 20, 148, 14, "Pixels + palette", 0, 1, KEY_RETURN);               // 1
  ...
  Window_set_normal_button(
    10, 71, 148, 14, "Some colors only", 6, 1, KEY_c); // 4
  Window_set_normal_button(
    10, 88, 148, 14, "Palette and remap", 13, 1, KEY_r);              // 5
...
void
Check_mode_button(short x_pos, short y_pos, byte state)
{
  byte color;
...
void
Display_modes_list(short list_start, short cursor_position)
{
  ...
  for (current_mode = list_start, index = 0;
       index < MODELIST_LINES && current_mode < Nb_video_modes;
       index++, current_mode++) {
    y_pos = 86 + (index << 3);
    ...
    snprintf(str,
             sizeof(str),
             "%4hu %4hu",
             Video_mode[current_mode].Width,
             Video_mode[current_mode].Height);
    ...
    if (Video_mode[current_mode].Fullscreen == 0)
      ...
      if (Video_mode[current_mode].Width * 3 ==
          Video_mode[current_mode].Height * 4)
        ratio = "    4:3";
      else if (Video_mode[current_mode].Width * 9 ==
               Video_mode[current_mode].Height * 16)
        ratio = "   16:9";
      else if (Video_mode[current_mode].Width * 10 ==
               Video_mode[current_mode].Height * 16)
        ratio = "  16:10";
      else if (Video_mode[current_mode].Width * 145 ==
               Video_mode[current_mode].Height * 192)
        ratio = "192:145";
      else if (Video_mode[current_mode].Width * 2 ==
               Video_mode[current_mode].Height * 3)
        ratio = "    3:2";
      else if (Video_mode[current_mode].Width * 3 ==
               Video_mode[current_mode].Height * 5)
        ratio = "    5:3";
      else if (Video_mode[current_mode].Width * 4 ==
               Video_mode[current_mode].Height * 5)
        ratio = "    5:4";
      else if (Video_mode[current_mode].Width * 16 ==
               Video_mode[current_mode].Height * 25)
        ratio = "  25:16";
      else
        ratio = "       ";
...
void
Scroll_list_of_modes(short list_start,
...
Button_Resolution(int btn)
{
  ...
  static const char* pixel_ratio_labels[PIXEL_MAX] = {
    "Normal    (1x1)", "Wide      (2x1)", "Tall      (1x2)",
    "Double    (2x2)", "Triple    (3x3)", "Wide2     (4x2)",
    "Tall2     (2x4)", "Tall3     (3x4)", "Quadruple (4x4)"
  };
  ...
  Window_set_scroller_button(
    271, 85, 81, Nb_video_modes, MODELIST_LINES, list_start); // 6

  chosen_pixel = Pixel_ratio;
  Print_in_window(12, 57, "Pixel size:", MC_Dark, MC_Light);
  pixel_button = Window_set_dropdown_button(108,
                                            55,
                                            17 * 8,
                                            11,
                                            17 * 8,
                                            pixel_ratio_labels[Pixel_ratio],
                                            1,
                                            0,
                                            1,
  ...
  for (temp = 0; temp < MODELIST_LINES && temp < Nb_video_modes; temp++)
    Window_set_normal_button(
      17, 86 + (temp << 3), 13, 7, "", 0, 1, KEY_NONE); // 8..17
  ...
  do {
    ...
    switch (clicked_button) {
      ...
      case 5: // Liste des modes
        ...
        if (temp < Nb_video_modes &&
            ((Mouse_K == 2) || (temp != cursor_position))) {
          Hide_cursor();
      ...
      default: // Boutons de tag des états des modes
        ...
        if (Video_mode[temp].Fullscreen ==
              1 && // On n'a pas le droit de cocher le mode fenêtré
    ...
    switch (Key) {
      case KEY_UP: // Haut
        ...
        break;
      case KEY_DOWN: // Bas
        if (cursor_position < (MODELIST_LINES - 1) &&
            cursor_position < (Nb_video_modes - 1))
          cursor_position++;
        else if (list_start < Nb_video_modes - MODELIST_LINES)
          list_start++;
        Scroll_list_of_modes(list_start, cursor_position, &selected_mode);
        Key = 0;
        break;
  ...
  if (clicked_button == 1) // OK
  {
    ...
    if ((Video_mode[selected_mode].State & 3) == 3 ||
        Init_mode_video(Video_mode[selected_mode].Width,
      ...
      Init_mode_video(Video_mode[Current_resolution].Width,
...
void
Button_Safety_resolution(int btn)
{
  ...
  Init_mode_video(Video_mode[0].Width,
                  Video_mode[0].Height,
...
void
Button_Draw_switch_mode(int btn)
{
  char icon;
...
void
Button_Empty_rectangle(int btn)
{
  (void)btn;
  Hide_cursor();
...
void
Button_Filled_rectangle(int btn)
{
  (void)btn;
  Hide_cursor();
...
Button_circle_ellipse(int btn)
{
  ...
  operation +=
    Selected_circle_ellipse_mode; // CIRCLE_CTR/CIRCLE_CRN/ELLIPSE_CTR/ELLIPSE_CRN;
  Hide_cursor();
...
void
Button_Circle_switch_mode(int btn)
{
  T_Dropdown_button dropdown;
  T_Dropdown_choice* item = NULL;
  int i;
  static const char* text[4] = { "Circle (center/radius)",
                                 "Circle (corners)",
                                 "Ellipse (center/radiuses)",
                                 "Ellipse (corners)" };
...
void
Draw_button_gradient_style(short x_pos, short y_pos, int technique)
{
  short line;
...
void
Load_gradient_data(int index)
{
  if (Main.backups->Pages->Gradients->Range[index].Start >
      Main.backups->Pages->Gradients->Range[index].End)
    Error(0);
...
static void
Draw_gradient_preview(short start_x,
                      short start_y,
                      short width,
                      short height,
                      int index)
...
static void
Tag_color_gradient_range(const T_Gradient_range* range)
{
...
static void
Print_color_cycling_details(short y_pos)
{
  char str[24];

  Num2str(
    Main.backups->Pages->Gradients->Range[Current_gradient].Speed, str, 3);
  Print_in_window(73 + 134, y_pos, str, MC_Black, MC_Light);
  if (Main.backups->Pages->Gradients->Range[Current_gradient].Speed > 0) {
    snprintf(str,
             sizeof(str),
             "Speed %7.4fHz %4ums",
             Main.backups->Pages->Gradients->Range[Current_gradient].Speed *
               0.2856,
             (unsigned)(1000.0 /
                        (Main.backups->Pages->Gradients->Range[Current_gradient]
                           .Speed *
                         0.2856)));
...
Button_Gradients(int btn)
{
  ...
  memcpy(&backup_gradients,
         Main.backups->Pages->Gradients,
         sizeof(T_Gradient_array));
  ...
  gradient_scroller =
    Window_set_scroller_button(218, 20, 75, 16, 1, Current_gradient); // 2
  // Slider for mix
  mix_scroller = Window_set_scroller_button(
    31,
    20,
    84,
    256,
  ...
  Window_set_normal_button(
    ...
    15,
    14,
    (Main.backups->Pages->Gradients->Range[Current_gradient].Inverse) ? "\033"
                                                                      : "\032",
    0,
    1,
    KEY_TAB); // 4
  // Technique
  Window_set_normal_button(
    8, 90, 15, 14, "", 0, 1, KEY_TAB | GFX2_MOD_SHIFT); // 5
  Draw_button_gradient_style(
    8, 90, Main.backups->Pages->Gradients->Range[Current_gradient].Technique);
  ...
  speed_scroller = Window_set_horizontal_scroller_button(
    79,
    111,
    150,
    COLOR_CYCLING_SPEED_MAX + 1,
    1,
  ...
  Print_in_window(
    11, 112, "Cycling", cycling_mode ? MC_Black : MC_Dark, MC_Light);
  ...
  first_color = last_color =
    (Main.backups->Pages->Gradients->Range[Current_gradient].Inverse)
      ? Main.backups->Pages->Gradients->Range[Current_gradient].End
      : Main.backups->Pages->Gradients->Range[Current_gradient].Start;
  Update_window_area(0, 0, Window_width, Window_height);
  ...
  do {
    ...
    if (changed_gradient_index) {
      ...
      mix_scroller->Position =
        Main.backups->Pages->Gradients->Range[Current_gradient].Mix;
      Window_draw_slider(mix_scroller);

      // Update speed
      speed_scroller->Position =
        Main.backups->Pages->Gradients->Range[Current_gradient].Speed;
    ...
    switch (clicked_button) {
      ...
      case 1: // Palette
        if ((Mouse_X != old_mouse_x) || (Mouse_Y != old_mouse_y) ||
            (Mouse_K != old_mouse_k)) {
          Hide_cursor();
          temp_color = (clicked_button == 1) ? Window_attribute2
                                             : Read_pixel(Mouse_X, Mouse_Y);
  ...
  if (clicked_button == 7) // Cancel
  {
    ...
    memcpy(Main.backups->Pages->Gradients,
           &backup_gradients,
           sizeof(T_Gradient_array));
  }
}
...
Button_Fill(int btn)
{
  ...
  if (Current_operation != OPERATION_FILL) {
    ...
    if (Current_operation != OPERATION_REPLACE) {
      ...
      Paintbrush_shape = PAINTBRUSH_SHAPE_POINT;
    } else if ((Mouse_Y < Menu_Y) && (Menu_is_visible) &&
  ...
  if (Paintbrush_shape == PAINTBRUSH_SHAPE_MISC) {
    ...
    for (y = 0; y < Paintbrush_height; y++)
      for (x = 0; x < Paintbrush_width; x++)
        if (Paintbrush_sprite[(y * MAX_PAINTBRUSH_SIZE) + x] !=
            Paintbrush[index].Sprite[y][x])
          return 0;
  }
  return 1;
}
...
void
Button_Paintbrush_menu(int btn)
{
  short clicked_button;
  short x_pos, y_pos;
  byte index;
  ...
  Window_set_dropdown_button(
    216, 158, 84, 14, 84, "Preset...", 0, 0, 1, RIGHT_SIDE | LEFT_SIDE, 1);
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_ROUND, "Round");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_SQUARE, "Square");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_HORIZONTAL_BAR, "Horizontal");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_VERTICAL_BAR, "Vertical");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_SLASH, "Slash");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_ANTISLASH, "Antislash");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_RANDOM, "Random");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_CROSS, "Cross");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_PLUS, "Plus");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_DIAMOND, "Diamond");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_SIEVE_ROUND, "Sieve Rnd");
  Window_dropdown_add_item(
    Window_dropdown_button_list, PAINTBRUSH_SHAPE_SIEVE_SQUARE, "Sieve Sqr");
  ...
  for (index = 0; index < NB_PAINTBRUSH_SPRITES; index++) {
    ...
    Window_set_dropdown_button(
      x_pos, y_pos, 20, 20, 28, NULL, 0, 0, 0, RIGHT_SIDE, 0);
  ...
  for (index = 0; index < BRUSH_CONTAINER_COLUMNS * BRUSH_CONTAINER_ROWS;
       index++) {
      ...
      if (Window_attribute2 != 1) {
        ...
        if (!Store_paintbrush(index)) {
          ...
          Display_cursor();
               ...
               Is_shortcut(Key, 0x100 + BUTTON_PAINTBRUSHES)) {
      Close_window();
      break;
...
void
Button_Brush_monochrome(int btn)
{
  Hide_cursor();
  ...
  for (mode = 1; mode < Nb_video_modes; mode++) {
    if (Video_mode[mode].Fullscreen && (Video_mode[mode].State & 3) < 2) {
      ...
      if ((Original_screen_X - TOLERANCE_X <= temp_x) &&
        ...
        if ((best_width <= temp_x) && (best_height <= temp_y) &&
...
void
Load_picture(enum CONTEXT_TYPE type)
{
  byte confirm;
  byte old_cursor_shape;
  int new_mode;
  ...
  switch (type) {
    case CONTEXT_MAIN_IMAGE:
      ...
      Init_context_layered_image(&context,
                                 Main.backups->Pages->Filename,
                                 Main.backups->Pages->File_directory);
    ...
    case CONTEXT_PALETTE:
      Init_context_layered_image(
        &context, "", Main.backups->Pages->File_directory);
  ...
  if (confirm) {
    ...
    if ((File_error == 1) || (Get_fileformat(Main.fileformat)->Palette_only)) {
      ...
      if (type == CONTEXT_MAIN_IMAGE) {
        if (Main.magnifier_mode && (Main.image_width > old_image_width ||
                                    Main.image_height > old_image_height)) {
          // disable magnifier
          Pixel_preview = Pixel_preview_normal;
        ...
        if ((Config.Auto_set_res) && (new_mode != Current_resolution)) {
          Init_mode_video(Video_mode[new_mode].Width,
        ...
        // In window mode, activate wide or tall pixels if the image says so.
                 ...
                 ((context.Ratio == PIXEL_WIDE && Pixel_ratio != PIXEL_WIDE &&
                   Pixel_ratio != PIXEL_WIDE2) ||
                  (context.Ratio == PIXEL_TALL && Pixel_ratio != PIXEL_TALL &&
          ...
          if (Main.image_width > old_image_width ||
              Main.image_height > old_image_height) {
            Main.offset_X = 0;
            Main.offset_Y = 0;
          }
...
Button_Reload(int btn)
{
  ...
  if ((!Main.image_is_modified) ||
      Confirmation_box("Discard unsaved changes ?")) {
    T_IO_Context context;
    ...
    if (File_error != 1) {
      ...
      // In window mode, activate wide or tall pixels if the image says so.
               ...
               ((context.Ratio == PIXEL_WIDE && Pixel_ratio != PIXEL_WIDE &&
                 Pixel_ratio != PIXEL_WIDE2) ||
                (context.Ratio == PIXEL_TALL && Pixel_ratio != PIXEL_TALL &&
                 Pixel_ratio != PIXEL_TALL2 && Pixel_ratio != PIXEL_TALL3))) {
        Init_mode_video(Video_mode[Current_resolution].Width,
                        Video_mode[Current_resolution].Height,
                        Video_mode[Current_resolution].Fullscreen,
                        context.Ratio);
...
static void
Backup_existing_file(const char* filename)
{
  ...
  p = Find_last_separator(
    new_filename); // pointer to the filename part (after directory)
  if (p == NULL)
    p = new_filename;
  else
    p++;
  ...
  if (strcmp(new_filename, filename)) {
    ...
    if ((File_exists(new_filename)) && (Remove_path(new_filename) != 0))
      File_error = 1;

    if ((!File_error) && (rename(filename, new_filename) != 0))
      File_error = 1;
  }
  free(new_filename);
}
...
void
Save_picture(enum CONTEXT_TYPE type)
{
  byte confirm;
  byte old_cursor_shape;
  T_IO_Context save_context;
  ...
  if (type == CONTEXT_MAIN_IMAGE) {
    ...
    Init_context_layered_image(
      &save_context, pal_filename, Main.backups->Pages->File_directory);
  ...
  if (confirm && File_exists(save_context.File_name)) {
    ...
    if (confirm && (Config.Backup)) {
      char* full_filename = Filepath_append_to_dir(save_context.File_directory,
                                                   save_context.File_name);
  ...
  if (confirm) {
    ...
    if (!File_error && type == CONTEXT_MAIN_IMAGE && !format->Palette_only &&
        (Main.backups->Pages->Nb_layers == 1 || format->Supports_layers)) {
      Main.image_is_modified = 0;
      Main.fileformat = save_context.Format;
      free(Main.backups->Pages->Filename);
      Main.backups->Pages->Filename =
        save_context.File_name; // "steal" string from heap
      save_context.File_name = NULL;
      free(Main.backups->Pages->Filename_unicode);
      Main.backups->Pages->Filename_unicode =
        save_context.File_name_unicode; // "steal" string
  ...
  if ((!file_already_exists) || Confirmation_box("Erase old file ?")) {
    ...
    if (!File_error) {
      ...
      Init_context_layered_image(&save_context,
                                 Main.backups->Pages->Filename,
                                 Main.backups->Pages->File_directory);
      save_context.File_name_unicode =
        Unicode_strdup(Main.backups->Pages->Filename_unicode);
      Save_image(&save_context);
      if (!File_error) {
        Main.image_is_modified = 0;
      }
...
void
Button_Lines_switch_mode(int btn)
{
  char icon;
...
void
Button_Unselect_colorpicker(int btn)
{
  (void)btn;
...
void
Button_Invert_foreback(int btn)
{
  byte temp_color;
...
Button_Magnify(int btn)
{
  ...
  if ((Current_operation == OPERATION_MAGNIFY) || (Main.magnifier_mode)) {
    ...
    Compute_magnifier_data();
    if ((!Config.Fast_zoom) || (Mouse_Y >= Menu_Y) ||
        Coming_from_zoom_factor_menu) {
      Coming_from_zoom_factor_menu = 0;
      Start_operation_stack(OPERATION_MAGNIFY);
    } else { /* Ceci est de la duplication de code de presque toute l'opération
                de */
      ...
      Clip_magnifier_offsets(&Main.magnifier_offset_X,
                             &Main.magnifier_offset_Y);
...
void
Button_Magnify_menu(int btn)
{
  T_Dropdown_button dropdown;
  T_Dropdown_choice* item;
  int i;
  static const char* text[NB_ZOOM_FACTORS] = { "x2",  "x3",  "x4",  "x5",
                                               "x6",  "x8",  "x10", "x12",
                                               "x14", "x16", "x18", "x20",
                                               "x24", "x28", "x32" };
  ...
  if ((!item) && (!Main.magnifier_mode) &&
      (Current_operation != OPERATION_MAGNIFY)) // Cancel
    Unselect_button(BUTTON_MAGNIFIER);

  Display_all_screen();
  Display_cursor();
  Update_rect(
    Main.separator_position, 0, Screen_width - Main.separator_position, Menu_Y);

  if ((item) && (!Main.magnifier_mode) &&
      (Current_operation != OPERATION_MAGNIFY)) // Passage en mode zoom
  {
...
void
Button_Unselect_magnifier(int btn)
{
  (void)btn;
  if (Main.magnifier_mode) {
    ...
    Main.offset_X =
      Main.magnifier_offset_X - ((Screen_width - Main.magnifier_width) >> 1);
    Main.offset_Y =
      Main.magnifier_offset_Y - ((Menu_Y - Main.magnifier_height) >> 1);
...
Button_Brush_FX(int btn)
{
  ...
  Window_set_normal_button(
    19, 46, 27, 14, "X\035", 0, 1, Config_Key[SPECIAL_FLIP_X][0]); // 2
  Window_set_normal_button(
    19, 61, 27, 14, "Y\022", 0, 1, Config_Key[SPECIAL_FLIP_Y][0]); // 3
  Window_set_normal_button(
    58, 46, 37, 14, "90\xb0", 0, 1, Config_Key[SPECIAL_ROTATE_90][0]); // 4
  Window_set_normal_button(
    96, 46, 37, 14, "180\xb0", 0, 1, Config_Key[SPECIAL_ROTATE_180][0]); // 5
  Window_set_normal_button(58,
                           61,
                           75,
                           14,
                           "any angle",
                           0,
                           1,
                           Config_Key[SPECIAL_ROTATE_ANY_ANGLE][0]); // 6
  Window_set_normal_button(
    145, 46, 67, 14, "Stretch", 0, 1, Config_Key[SPECIAL_STRETCH][0]); // 7
  Window_set_normal_button(
    145, 61, 67, 14, "Distort", 0, 1, Config_Key[SPECIAL_DISTORT][0]); // 8
  Window_set_normal_button(155,
                           99,
                           131,
                           14,
                           "Recolorize",
                           0,
                           1,
                           Config_Key[SPECIAL_RECOLORIZE_BRUSH][0]); // 9
  Window_set_normal_button(155,
                           117,
                           131,
                           14,
                           ...
                           0,
                           1,
                           Config_Key[SPECIAL_GET_BRUSH_COLORS][0]); // 10

  // Boutons représentant les coins du brush handle: (HG,HD,C,BG,BD)
  Window_set_normal_button(
    75, 90, 11, 11, "", 0, 1, Config_Key[SPECIAL_TOP_LEFT_ATTACHMENT][0]); // 11
  Window_set_normal_button(103,
                           90,
                           11,
                           11,
                           "",
                           0,
                           1,
                           Config_Key[SPECIAL_TOP_RIGHT_ATTACHMENT][0]); // 12
  Window_set_normal_button(
    89, 104, 11, 11, "", 0, 1, Config_Key[SPECIAL_CENTER_ATTACHMENT][0]); // 13
  Window_set_normal_button(75,
                           118,
                           11,
                           11,
                           "",
                           0,
                           1,
                           Config_Key[SPECIAL_BOTTOM_LEFT_ATTACHMENT][0]); // 14
  Window_set_normal_button(
    103,
    118,
    11,
    ...
    0,
    1,
    Config_Key[SPECIAL_BOTTOM_RIGHT_ATTACHMENT][0]); // 15

  Window_set_normal_button(
    224, 46, 67, 14, "Outline", 0, 1, Config_Key[SPECIAL_OUTLINE][0]); // 16
  Window_set_normal_button(
    224, 61, 67, 14, "Nibble", 0, 1, Config_Key[SPECIAL_NIBBLE][0]); // 17

  Window_set_normal_button(
    7, 141, 60, 14, "Load", 0, 1, Config_Key[SPECIAL_LOAD_BRUSH][0]); // 18
  Window_set_normal_button(
    70, 141, 60, 14, "Save", 0, 1, Config_Key[SPECIAL_SAVE_BRUSH][0]); // 19
  ...
  switch (clicked_button) {
    case 2: // Flip X
      ...
      Remap_general_lowlevel(Brush_colormap,
...
void
Button_Curves_switch_mode(int btn)
{
...
void
Refresh_airbrush_settings(byte selected_color, byte update_slider)
{
  char str[4];

  if (update_slider) {
    Window_scroller_button_list->Position =
      49 - Airbrush_multi_flow[selected_color];
    Window_draw_slider(Window_scroller_button_list);
  }
  ...
  Update_window_area(
    Window_palette_button_list->Pos_X + 4 + (selected_color >> 4) * 10,
...
Button_Airbrush_menu(int btn)
{
  ...
  Window_set_scroller_button(
    178, 62, 74, 50, 1, 49 - Airbrush_multi_flow[selected_color]); // 3
    ...
    switch (clicked_button) {
      ...
      case 4: // Palette
        if ((Mouse_X != old_mouse_x) || (Mouse_Y != old_mouse_y) ||
            (Mouse_K != old_mouse_k)) {
          Hide_cursor();
          Stencil_tag_color(selected_color,
                            (Airbrush_multi_flow[selected_color]) ? MC_Black
                                                                  : MC_Light);
          Stencil_update_color(selected_color);
          // Mettre la couleur sélectionnée à jour suivant le click
          selected_color = (clicked_button == 4) ? Window_attribute2
                                                 : Read_pixel(Mouse_X, Mouse_Y);
          if (Mouse_K == 2)
            Airbrush_multi_flow[selected_color] = 0;
          else if (Airbrush_multi_flow[selected_color] == 0)
...
void
Button_Filled_polyform(int btn)
{
  (void)btn;
  Hide_cursor();
...
void
Display_effect_sprite(int sprite_number, short start_x, short start_y)
{
  ...
  Update_rect(ToWinX(start_x),
              ToWinY(start_y),
              EFFECT_SPRITE_WIDTH * Menu_factor_X,
              EFFECT_SPRITE_HEIGHT * Menu_factor_Y);
}

void
Display_effect_state(short x, short y, char* label, byte state)
{
  Window_rectangle(x + 22, y + 5, 12, 8, MC_Light);

  Print_in_window(
    x + 22, y + 5, label, (state) ? MC_White : MC_Black, MC_Light);
  if (state)
...
Button_Effects(int btn)
{
  ...
  Window_set_normal_button(
    C1, L2, 16, 16, "", 0, 1, Config_Key[SPECIAL_SHADE_MODE][0]); // 1
  Window_set_normal_button(
    C1, L3, 16, 16, "", 0, 1, Config_Key[SPECIAL_QUICK_SHADE_MODE][0]); // 2
  Window_set_normal_button(
    C1, L4, 16, 16, "", 0, 1, Config_Key[SPECIAL_COLORIZE_MODE][0]); // 3
  Window_set_normal_button(
    C1, L5, 16, 16, "", 0, 1, Config_Key[SPECIAL_SMOOTH_MODE][0]); // 4
  Window_set_normal_button(
    C2, L5, 16, 16, "", 0, 1, Config_Key[SPECIAL_SMEAR_MODE][0]); // 5
  Window_set_normal_button(
    C2, L1, 16, 16, "", 0, 1, Config_Key[SPECIAL_STENCIL_MODE][0]); // 6
  Window_set_normal_button(
    C2, L2, 16, 16, "", 0, 1, Config_Key[SPECIAL_MASK_MODE][0]); // 7
  Window_set_normal_button(
    C2, L3, 16, 16, "", 0, 1, Config_Key[SPECIAL_SIEVE_MODE][0]); // 8
  Window_set_normal_button(
    C3, L2, 16, 16, "", 0, 1, Config_Key[SPECIAL_GRID_MODE][0]); // 9
  Window_set_normal_button(
    C3, L4, 16, 16, "", 0, 1, Config_Key[SPECIAL_TILING_MODE][0]); // 10
  ...
  Window_set_normal_button(
    C2, L4, 16, 16, "", 0, 1, Config_Key[SPECIAL_FORMAT_CHECKER_MENU][0]); // 14
  Window_set_normal_button(
    C3, L3, 16, 16, "", 0, 1, Config_Key[SPECIAL_TILEMAP_MODE][0]); // 15

  // "Grid" frame
  Window_display_frame_mono(C3 - 5, L1 + 8, 86, 88, MC_Dark);
  Window_rectangle(C3 - 1, L1 + 2, 52, 14, MC_Light);

  Window_set_normal_button(C3 + 1,
                           L1 + 2,
                           14,
                           14,
                           Show_grid ? "X" : " ",
                           0,
                           1,
                           Config_Key[SPECIAL_SHOW_GRID][0]); // 16
  ...
  do {
    ...
    switch (clicked_button) {
      ...
      case 14: // Constraint checker/enforcer
        if (Window_attribute1 == LEFT_SIDE &&
            Selected_Constraint_Mode > IMAGE_MODE_ANIMATION) {
          Button_Constraint_mode();
          Hide_cursor();
          Display_effect_state(C2,
                               L4,
                               "8 bit",
                               Main.backups->Pages->Image_mode >
      ...
      case 16: // Show grid
        ...
        Hide_cursor();
        Print_in_window(
          C3 + 4, L1 + 5, Show_grid ? "X" : " ", MC_Black, MC_Light);
        Display_cursor();
    }
...
void
Draw_one_font_name(word x, word y, word index, byte highlighted)
{
  Print_in_window(
    x, y, Font_label(index), MC_Black, (highlighted) ? MC_Dark : MC_Light);
}
...
Button_Text(int btn)
{
  ...
  // AA
  Window_set_normal_button(
    188, 58, 13, 11, antialias ? "X" : " ", 0, 1, KEY_a); // 2
  Print_in_window_underscore(206, 60, "AntiAlias", MC_Dark, MC_Light, 5);
  // Bold
  Window_set_normal_button(
    188, 72, 13, 11, is_bold ? "X" : " ", 0, 1, KEY_b); // 3
  Print_in_window_underscore(206, 75, "Bold", MC_Dark, MC_Light, 1);
  // Italic
  Window_set_normal_button(
    188, 86, 13, 11, is_italic ? "X" : " ", 0, 1, KEY_i); // 4
  Print_in_window_underscore(206, 89, "Italic", MC_Dark, MC_Light, 1);

  // Scroller des fontes
  font_scroller = Window_set_scroller_button(
    165, 35, NB_FONTS * 8, Font_count(), NB_FONTS, list_start); // 5
  // Liste des fontes disponibles
  font_list_button =
    Window_set_special_button(8, 35, 152, NB_FONTS * 8, 0); // 6
  ...
  // List of fonts
  font_list = Window_set_list_button(
    font_list_button, font_scroller, Draw_one_font_name, 2); // 13
  ...
  // str texte
  if (str_unicode[0] == 0) {
    Str_to_utf8(str_unicode, str, sizeof(str) - 1);
  }
  Window_input_content_unicode(input_text_button, str_unicode);
  ...
  while (1) {
    ...
    if (preview_is_needed) {
      ...
      free(new_brush);
      new_brush = Render_text(preview_string,
                              selected_font_index,
                              font_size,
                              antialias,
                              is_bold,
      ...
      if (antialias && is_truetype)
        ...
        long l = text_palette[Fore_color].R + text_palette[Fore_color].G +
                 text_palette[Fore_color].B;
        Window_rectangle(8, 106, 273, 50, l > 128 * 3 ? MC_Black : MC_Light);
      } else {
        long l = text_palette[Back_color].R + text_palette[Back_color].G +
                 text_palette[Back_color].B;
          ...
          if (remapped_brush) {
            ...
            for (color = 0; color <= 255; color++)
              if (colmap[color] != 0) {
                ...
                // if (r==Main.palette[color].R && g==Main.palette[color].G &&
                // b==Main.palette[color].B)
                //   colmap[color]=color;
                // else
                colmap[color] =
                  Best_color_perceptual_except(r, g, b, Back_color);
              }

            colmap[Back_color] = Back_color;
            Remap_general_lowlevel(colmap,
                                   new_brush,
                                   remapped_brush,
                                   new_width,
                                   new_height,
      ...
      Update_window_area(preview_button->Pos_X,
    ...
    switch (clicked_button) {
      case 1: // Texte saisi
        // Readline_ex(50, 20, str, 29, sizeof(str) - 1, INPUT_TYPE_STRING, 0);
        Readline_ex_unicode(
          50, 20, str, str_unicode, 29, sizeof(str) - 1, INPUT_TYPE_STRING, 0);
        preview_is_needed = 1;
        break;
...
void
Display_stored_brush_in_window(word x_pos, word y_pos, int index)
{
  if (Brush_container[index].Paintbrush_shape < PAINTBRUSH_SHAPE_MAX) {
    ...
    // Determine draw offset (small brushes are stacked on corner of their
    // preview)
    if (Brush_container[index].Width < BRUSH_CONTAINER_PREVIEW_WIDTH)
      offset_x =
        (BRUSH_CONTAINER_PREVIEW_WIDTH - Brush_container[index].Width) / 2;
    if (Brush_container[index].Height < BRUSH_CONTAINER_PREVIEW_HEIGHT)
      offset_y =
        (BRUSH_CONTAINER_PREVIEW_HEIGHT - Brush_container[index].Height) / 2;
    // Determine corner pixel of paintbrush to draw (if bigger than preview
    // area)
    //
    ...
    // Draw up to 16x16
    for (y = 0; y < Brush_container[index].Height &&
                y < BRUSH_CONTAINER_PREVIEW_HEIGHT;
         y++) {
      for (x = 0; x < Brush_container[index].Width &&
                  x < BRUSH_CONTAINER_PREVIEW_WIDTH;
           x++) {
        byte color;
        if (Brush_container[index].Paintbrush_shape <= PAINTBRUSH_SHAPE_MISC)
          ...
          color = Brush_container[index]
                    .Colormap[Brush_container[index].Thumbnail[y][x]];
        Pixel_in_window(x_pos + x + offset_x, y_pos + y + offset_y, color);
      }
    }
    Update_window_area(x_pos,
                       y_pos,
                       BRUSH_CONTAINER_PREVIEW_WIDTH,
                       BRUSH_CONTAINER_PREVIEW_HEIGHT);
  }
...
Store_brush(int index)
{
  ...
  if (Paintbrush_shape <= PAINTBRUSH_SHAPE_MISC) {
    ...
    for (y = 0; y < BRUSH_CONTAINER_PREVIEW_HEIGHT && y < Paintbrush_height;
         y++)
      for (x = 0; x < BRUSH_CONTAINER_PREVIEW_WIDTH && x < Paintbrush_width;
           x++)
        Brush_container[index].Thumbnail[y][x] =
          Paintbrush_sprite[((y + brush_offset_y) * MAX_PAINTBRUSH_SIZE) + x +
                            brush_offset_x];
    // Re-init the rest
    ...
    for (y = 0; y < BRUSH_CONTAINER_PREVIEW_HEIGHT && y < Brush_height; y++)
      for (x = 0; x < BRUSH_CONTAINER_PREVIEW_WIDTH && x < Brush_width; x++)
        Brush_container[index].Thumbnail[y][x] =
          (Brush[y * Brush_width + x] != Back_color);
    // Re-init the rest
    Brush_container[index].Transp_color = 0;
    ...
    if (buffer) {
      ...
      memcpy(Brush_container[index].Brush,
             Brush_original_pixels,
             Brush_height * Brush_width);
      memcpy(Brush_container[index].Palette,
      ...
      if (Brush_width > BRUSH_CONTAINER_PREVIEW_WIDTH ||
          Brush_height > BRUSH_CONTAINER_PREVIEW_HEIGHT) {
        // Scale
        Rescale(Brush_original_pixels,
                Brush_width,
                Brush_height,
                (byte*)(Brush_container[index].Thumbnail),
                BRUSH_CONTAINER_PREVIEW_WIDTH,
                BRUSH_CONTAINER_PREVIEW_HEIGHT,
                0,
                0);
        ...
        // Direct copy
        Copy_part_of_image_to_another(Brush_original_pixels,
                                      0,
                                      0,
                                      Brush_width,
                                      Brush_height,
  ...
  if (Paintbrush[index].Width <= PAINTBRUSH_WIDTH &&
    ...
    for (y_pos = 0; y_pos < Paintbrush_height; y_pos++)
      for (x_pos = 0; x_pos < Paintbrush_width; x_pos++)
        Paintbrush_sprite[(y_pos * MAX_PAINTBRUSH_SIZE) + x_pos] =
          Paintbrush[index].Sprite[y_pos][x_pos];
  } else {
...
Store_paintbrush(int index)
{
  ...
  if (Paintbrush_shape <= PAINTBRUSH_SHAPE_MISC) {
    ...
    for (y_pos = 0; y_pos < Paintbrush_height && y_pos < PAINTBRUSH_HEIGHT;
         y_pos++)
      for (x_pos = 0; x_pos < Paintbrush_width && x_pos < PAINTBRUSH_WIDTH;
           x_pos++)
        Paintbrush[index].Sprite[y_pos][x_pos] =
          Paintbrush_sprite[((y_pos + y_off) * MAX_PAINTBRUSH_SIZE) +
                            (x_pos + x_off)];
    ...
    Paintbrush[index].Offset_X =
      Brush_offset_X * Paintbrush[index].Width / Brush_width;
    Paintbrush[index].Offset_Y =
      Brush_offset_Y * Paintbrush[index].Height / Brush_height;

    for (y_pos = 0; y_pos < Brush_height && y_pos < PAINTBRUSH_HEIGHT; y_pos++)
      for (x_pos = 0; x_pos < Brush_width && x_pos < PAINTBRUSH_WIDTH; x_pos++)
        Paintbrush[index].Sprite[y_pos][x_pos] =
          Brush[(y_pos * Brush_width) + x_pos] != Back_color;
...
Restore_brush(int index)
{
  ...
  if (shape <= PAINTBRUSH_SHAPE_MISC) {
    ...
    if (Paintbrush_width <= BRUSH_CONTAINER_PREVIEW_WIDTH &&
      ...
      for (y_pos = 0; y_pos < Paintbrush_height; y_pos++)
        for (x_pos = 0; x_pos < Paintbrush_width; x_pos++)
          Paintbrush_sprite[(y_pos * MAX_PAINTBRUSH_SIZE) + x_pos] =
            Brush_container[index].Thumbnail[y_pos][x_pos];
  ...
  if (shape == PAINTBRUSH_SHAPE_COLOR_BRUSH ||
    ...
    Paintbrush_shape = shape;
    if (!Realloc_brush(Brush_container[index].Width,
                       Brush_container[index].Height,
                       NULL,
                       NULL)) {
      // Recover pixels
      memcpy(Brush_original_pixels,
             Brush_container[index].Brush,
             (long)Brush_height * Brush_width);
      // Grab palette
      memcpy(Brush_original_palette,
             Brush_container[index].Palette,
             sizeof(T_Palette));
      // Recover colormap
...
Button_Brush_container(void)
{
  ...
  index = 0;
  for (index = 0; index < BRUSH_CONTAINER_ROWS * BRUSH_CONTAINER_COLUMNS;
       index++) {
    x_pos =
      (index % BRUSH_CONTAINER_COLUMNS) * (BRUSH_CONTAINER_PREVIEW_WIDTH + 8) +
      7;
    y_pos =
      (index / BRUSH_CONTAINER_COLUMNS) * (BRUSH_CONTAINER_PREVIEW_HEIGHT + 8) +
      18;
    Window_set_normal_button(x_pos,
                             y_pos,
  ...
  do {
    ...
    if (clicked_button > 1) {
      ...
      if (Window_attribute1 == RIGHT_SIDE) {
        ...
        x_pos = (index % BRUSH_CONTAINER_COLUMNS) *
                  (BRUSH_CONTAINER_PREVIEW_WIDTH + 8) +
                7;
        y_pos = (index / BRUSH_CONTAINER_COLUMNS) *
                  (BRUSH_CONTAINER_PREVIEW_HEIGHT + 8) +
                18;
...
byte
Any_effect_active(void)
{
  return Shade_mode || Quick_shade_mode || Colorize_mode || Smooth_mode ||
         Tiling_mode || Smear_mode || Stencil_mode || Mask_mode || Sieve_mode ||
         Snap_mode || Main.tilemap_mode ||
         (Main.backups->Pages->Image_mode > IMAGE_MODE_ANIMATION);
}
