...
/// This table holds pointers to the saved window backgrounds. We can have up to
/// 8 windows open at a time.
static byte* Window_background[8];

/// Save a screen block (usually before erasing it with a new window or a
/// dropdown menu)
static void
Save_background(byte** buffer, int x_pos, int y_pos, int width, int height)
{
  int index;
  if (*buffer != NULL) {
    GFX2_Log(
      GFX2_WARNING, "%s() WARNING : buffer already allocated !!!\n", __func__);
    free(*buffer);
  }
  *buffer =
    (byte*)malloc(width * Menu_factor_X * height * Menu_factor_Y * Pixel_width);
  if (*buffer == NULL) {
    Error(0);
    return;
  }
  for (index = 0; index < (height * Menu_factor_Y); index++)
    Read_line(x_pos,
              y_pos + index,
              width * Menu_factor_X,
              (*buffer) + ((int)index * width * Menu_factor_X * Pixel_width));
}

/// Restores a screen block
static void
Restore_background(byte** buffer, int x_pos, int y_pos, int width, int height)
{
  int index;
  if (*buffer == NULL)
    return;
  for (index = 0; index < height * Menu_factor_Y; index++)
    Display_line_fast(x_pos,
                      y_pos + index,
                      width * Menu_factor_X,
                      *buffer +
                        ((int)index * width * Menu_factor_X * Pixel_width));
  free(*buffer);
  *buffer = NULL;
}

/// Draw a pixel in a saved screen block (when you sort colors in the palette,
/// for example)
void
Pixel_background(int x_pos, int y_pos, byte color)
{
  int x_repetition = Pixel_width;
  while (x_repetition--)
    (Window_background[0][x_pos * Pixel_width + x_repetition +
                          y_pos * Window_width * Pixel_width * Menu_factor_X]) =
      color;
}

/// Guess the number of the button that was just clicked
int
Button_under_mouse(void)
{
  int btn_number;
  short x_pos;
  short y_pos;
  ...
  for (current_menu = 0; current_menu < MENUBAR_COUNT; current_menu++) {
    if (Menu_bars[current_menu].Visible) {
      if (Mouse_Y >= Menu_Y + Menu_factor_Y * (Menu_bars[current_menu].Top) &&
          Mouse_Y < Menu_Y + Menu_factor_Y * (Menu_bars[current_menu].Top +
                                              Menu_bars[current_menu].Height))
        break;
    }
  }
  ...
  if (current_menu == 0)
    ...
    first_button = Menu_bars[current_menu - 1].Last_button_index + 1;

  for (btn_number = first_button;
       btn_number <= Menu_bars[current_menu].Last_button_index;
       btn_number++) {
    switch (Buttons_Pool[btn_number].Shape) {
      ...
      case BUTTON_SHAPE_RECTANGLE:
        ...
        if ((x_pos >= Buttons_Pool[btn_number].X_offset) &&
            ...
            (x_pos <= Buttons_Pool[btn_number].X_offset +
                        Buttons_Pool[btn_number].Width) &&
            (y_pos <= Buttons_Pool[btn_number].Y_offset +
                        Buttons_Pool[btn_number].Height))
          return btn_number;
        break;
      ...
      case BUTTON_SHAPE_TRIANGLE_TOP_LEFT:
        if ((x_pos >= Buttons_Pool[btn_number].X_offset) &&
            (y_pos >= Buttons_Pool[btn_number].Y_offset) &&
            (x_pos + y_pos - (short)Buttons_Pool[btn_number].Y_offset -
               (short)Buttons_Pool[btn_number].X_offset <=
             Buttons_Pool[btn_number].Width))
          return btn_number;
        break;
...
void
Draw_menu_button(byte btn_number, byte pressed)
{
  word start_x;
  word start_y;
  word width;
  ...
  for (current_menu = 0; current_menu < MENUBAR_COUNT; current_menu++) {
    ...
    if (Menu_bars[current_menu].Last_button_index >= btn_number &&
        (current_menu == 0 ||
         Menu_bars[current_menu - 1].Last_button_index < btn_number)) {
      break;
    }
  }
  ...
  if (icon == -1) {
    ...
    bitmap = &(
      Menu_bars[current_menu]
        .Skin[pressed][start_y * Menu_bars[current_menu].Skin_width + start_x]);
  } else {
    // From Menu_buttons
      ...
      for (y_pos = 0; y_pos < height; y_pos++)
        for (x_pos = 0; x_pos < width; x_pos++) {
          ...
          Pixel_in_menu_and_skin(
            current_menu, start_x + x_pos, start_y + y_pos, color);
        }
      break;
    case BUTTON_SHAPE_TRIANGLE_TOP_LEFT:
  ...
  if (btn_number == BUTTON_PAINTBRUSHES &&
    ...
    for (; ((y_pos < Paintbrush_height) && (menu_y_pos < 16));
         menu_y_pos++, y_pos++)
      for (menu_x_pos = menu_start_x, x_pos = start_x;
           ((x_pos < Paintbrush_width) && (menu_x_pos < 15));
           menu_x_pos++, x_pos++) {
        if (Paintbrush_sprite[(y_pos * MAX_PAINTBRUSH_SIZE) + x_pos])
          Pixel_in_menu_and_skin(
            MENUBAR_TOOLS, menu_x_pos, menu_y_pos, MC_Black);
      }
  }

  if (Menu_is_visible && Menu_bars[current_menu].Visible)
    Update_rect(
      Menu_factor_X * (Buttons_Pool[btn_number].X_offset),
      (Buttons_Pool[btn_number].Y_offset + Menu_bars[current_menu].Top) *
          Menu_factor_Y +
        Menu_Y,
      width * Menu_factor_X,
      height * Menu_factor_Y);
  ...
  /*
    ...
    switch(Buttons_Pool[btn_number].Shape)
    {
      ...
      case BUTTON_SHAPE_RECTANGLE  :
        ...
        for (y_pos=start_y+1;y_pos<=end_y-1;y_pos++)
        {
          ...
          Pixel_in_menu_and_skin(current_menu, end_x, y_pos,
    color_bottom_right);
        }
        // On colorie la partie basse
        for (x_pos=start_x+1;x_pos<=end_x;x_pos++)
        {
          Pixel_in_menu_and_skin(current_menu, x_pos, end_y,
    color_bottom_right);
        }
        break;
      case BUTTON_SHAPE_TRIANGLE_TOP_LEFT:
        ...
        {
          Pixel_in_menu_and_skin(current_menu, start_x+x_pos, start_y,
    color_top_left); Pixel_in_menu_and_skin(current_menu, start_x,
    start_y+x_pos, color_top_left);
        }
        // On colorie la diagonale
        for (x_pos=1;x_pos<Buttons_Pool[btn_number].Width;x_pos++)
        {
          Pixel_in_menu_and_skin(current_menu, start_x+x_pos, end_y-x_pos,
    color_bottom_right);
        }
        break;
      case BUTTON_SHAPE_TRIANGLE_BOTTOM_RIGHT:
    ...
    color_top_left);
        ...
        for (x_pos=0;x_pos<Buttons_Pool[btn_number].Width;x_pos++)
        {
          Pixel_in_menu_and_skin(current_menu, end_x-x_pos, end_y,
    color_bottom_right); Pixel_in_menu_and_skin(current_menu, end_x,
    end_y-x_pos, color_bottom_right);
        }
    }
    if (Menu_is_visible && Menu_bars[current_menu].Visible)
    {
...
/// Deselect a button
void
Unselect_button(int btn_number)
{
...
/// Select a button and disable all his family (for example, selecting
/// "freehand" unselect "curves", "lines", ...)
void
Select_button(int btn_number, byte click)
{
  int family;
  int b;
  int icon;
  ...
  if (icon != -1) {
    ...
    else if (Buttons_Pool[btn_number].Shape ==
  ...
  switch (family) {
    ...
    case FAMILY_INTERRUPTION: // Petit cas spécial dans la famille
                              // "Interruption":
    ...
    default:
      ...
      for (b = 0; b < NB_BUTTONS; b++)
        ...
        if ((b != btn_number) &&
            (Buttons_Pool[b].Family == FAMILY_INTERRUPTION)
            // Et que ce n'est pas la loupe, ou alors qu'on n'est pas en mode
            // loupe
      ...
      for (b = 0; b < NB_BUTTONS; b++)
        // S'il est de la même famille
        if ((b != btn_number) && (Buttons_Pool[b].Family == family))
          // Alors on désenclenche le bouton
          Unselect_button(b);
  }
...
Move_separator(void)
{
  ...
  Display_cursor();
  Update_rect(Main.separator_position,
              0,
              abs(Main.separator_position - Main.X_zoom) + 1,
              Menu_Y);
  ...
  while (Mouse_K) {
    if (Mouse_X != old_mouse_x) {
      ...
      if (Main.X_zoom != old_x_zoom) {
        ...
        Update_rect(old_main_separator_position,
                    0,
                    abs(old_main_separator_position - old_x_zoom) + 1,
                    Menu_Y);
...
Status_print_palette_color(byte color)
{
  ...
  i = snprintf(str,
               sizeof(str),
               "%s%d (%d,%d,%d)",
               Buttons_Pool[BUTTON_CHOOSE_COL].Tooltip,
               color,
...
static void
Layer_preview_on(int* preview_is_visible)
{
  int x, y;
  short layer;
  ...
  if (!*preview_is_visible && layercount > 1) {
    ...
    previewH = previewW * Main.image_height / Main.image_width * Menu_factor_X /
               Menu_factor_Y;
    if (previewH > Screen_height / 4) {
      previewH = Screen_height / 4;
      previewW = Main.image_width * previewH / Main.image_height *
                 Menu_factor_Y / Menu_factor_X;
    }

    Open_popup((Buttons_Pool[BUTTON_LAYER_SELECT].X_offset + 2) * Menu_factor_X,
               Menu_Y - previewH * Menu_factor_Y,
               Buttons_Pool[BUTTON_LAYER_SELECT].Width,
               previewH);
    ...
    for (layer = 0; layer < layercount; ++layer) {
      ...
      for (y = 0; y < previewH * Pixel_height * Menu_factor_Y - 1; y++)
        for (x = 0; x < previewW * Pixel_width * Menu_factor_X - 1; x++) {
          int imgx =
            x * Main.image_width / (previewW * Pixel_width * Menu_factor_X - 1);
          int imgy = y * Main.image_height /
                     (previewH * Pixel_height * Menu_factor_Y - 1);
          // Use Pixel_simple() in order to get highest resolution
          Pixel_simple(
            x + ((layer * Layer_button_width + offset) * Menu_factor_X +
                 Window_pos_X) *
                  Pixel_width,
...
void
Layer_preview_off(int* preview_is_visible)
{
...
Main_handler(void)
{
  ...
  int prev_button_number =
    0;        // Numéro de bouton de menu sur lequel on était précédemment
  byte blink; // L'opération demande un effacement du curseur
  int key_index; // index du tableau de touches spéciales correspondant à la
                 // touche enfoncée
  byte temp;
  ...
  do {
    ...
    if (Get_input(0)) {
      ...
      if (Pan_shortcut_pressed && Current_operation != OPERATION_PAN_VIEW &&
        ...
        Display_cursor();
        action++;
      } else if (Key == SHORTCUT_PASTE) /// @todo create a SPECIAL_ACTIONS for
                                        /// Copy and Paste ?
      {
        ...
        for (key_index = SPECIAL_CLICK_RIGHT + 1;
             key_index < NB_SPECIAL_SHORTCUTS;
             key_index++) {
          if (Is_shortcut(Key, key_index)) {
            ...
            switch (key_index) {
              ...
              case SPECIAL_BIGGER_PAINTBRUSH: // Grossir le pinceau
                ...
                action++;
                break;
              case SPECIAL_NEXT_USER_FORECOLOR: // Next user-defined foreground
                                                // color
                Special_next_user_forecolor();
                action++;
                break;
              case SPECIAL_PREVIOUS_USER_FORECOLOR: // Previous user-defined
                                                    // foreground color
                Special_previous_user_forecolor();
                action++;
                break;
              case SPECIAL_NEXT_USER_BACKCOLOR: // Next user-defined background
                                                // color
                Special_next_user_backcolor();
                action++;
                break;
              case SPECIAL_PREVIOUS_USER_BACKCOLOR: // Previous user-defined
                                                    // background color
            ...
            switch (key_index) {
              ...
              case SPECIAL_CONTINUOUS_DRAW: // Continuous freehand drawing
                Select_button(BUTTON_DRAW, LEFT_SIDE);
                // ATTENTION CE TRUC EST MOCHE ET VA MERDER SI ON SE MET A
                // UTILISER DES BOUTONS POPUPS
              ...
              case SPECIAL_FLIP_X: // Flip X
                Hide_cursor();
                Flip_X_lowlevel(
                  Brush_original_pixels, Brush_width, Brush_height);
              ...
              case SPECIAL_TOP_RIGHT_ATTACHMENT: // Top-right brush attachment
                ...
                action++;
                break;
              case SPECIAL_BOTTOM_LEFT_ATTACHMENT: // Bottom-left brush
                                                   // attachment
                Hide_cursor();
                Brush_offset_X = 0;
                Brush_offset_Y = (Brush_height - 1);
                ...
                action++;
                break;
              case SPECIAL_BOTTOM_RIGHT_ATTACHMENT: // Bottom right brush
                                                    // attachment
                Hide_cursor();
                Brush_offset_X = (Brush_width - 1);
                Brush_offset_Y = (Brush_height - 1);
              ...
              case SPECIAL_EXCLUDE_COLORS_MENU: // Exclude colors menu
                Menu_tag_colors("Tag colors to exclude",
                                Exclude_color,
                                &temp,
                                1,
                                NULL,
              ...
              case SPECIAL_LAYER8_SELECT:
                Layer_activate((key_index - SPECIAL_LAYER1_SELECT) / 2,
                               LEFT_SIDE);
                action++;
                break;
              ...
              case SPECIAL_LAYER8_TOGGLE:
                Layer_activate((key_index - SPECIAL_LAYER1_TOGGLE) / 2,
                               RIGHT_SIDE);
                action++;
                break;
    ...
    if (Cursor_in_menu) {
      ...
      if ((button_index != prev_button_number) || (!Cursor_in_menu_previous) ||
        ...
        if (button_index == -1) {
          if (Menu_is_visible) {
            ...
            /*if (Gfx->Hover_effect && prev_button_number > -1 &&
              !Buttons_Pool[prev_button_number].Pressed)
              Draw_menu_button(prev_button_number, BUTTON_RELEASED);
            */
            Block(18 * Menu_factor_X,
                  Menu_status_Y,
                  192 * Menu_factor_X,
                  Menu_factor_Y << 3,
                  MC_Light);
            Update_rect(18 * Menu_factor_X,
                        Menu_status_Y,
                        192 * Menu_factor_X,
                        Menu_factor_Y << 3);
            Display_cursor();
          ...
          if ((prev_button_number != BUTTON_CHOOSE_COL) ||
              (temp_color != First_color_in_palette) || (Old_MX != Mouse_X) ||
              (Old_MY != Mouse_Y)) {
            ...
            if (button_index != BUTTON_CHOOSE_COL) {
              ...
              */
              ...
              Display_cursor();
            } else { // Le curseur est-il sur une couleur de la palette?
              int color;
    ...
    if ((!Cursor_in_menu) && (Menu_is_visible) &&
        (Old_MY != Mouse_Y || Old_MX != Mouse_X || Key ||
         Mouse_K)) // On ne met les coordonnées à jour que si l'utilisateur a
                   // fait un truc
    {
    ...
    if (Cursor_in_menu) {
      ...
      blink = Operation[Current_operation][Mouse_K_unique][Operation_stack_size]
                .Hide_cursor;

      if (blink)
        Hide_cursor();

      Operation[Current_operation][Mouse_K_unique][Operation_stack_size]
        .Action();
 ...
 */
void
Open_window(word width, word height, const char* title)
{
  size_t title_length;
  ...
  if (width > 320 || height > 200)
    GFX2_Log(
      GFX2_WARNING,
      "Open_window(%d, %d, \"%s\") maximum size for windows is 320x200 !\n",
      width,
      height,
      title);
  if (width > 320)
  ...
  Save_background(&(Window_background[Windows_open - 1]),
                  Window_pos_X,
                  Window_pos_Y,
                  width,
                  height);
  ...
  Print_in_window_limited((width - (title_length << 3)) >> 1,
                          4,
                          title,
                          title_length,
                          MC_Black,
...
Close_window(void)
{
  ...
  if (Windows_open != 1) {
    ...
    Restore_background(&Window_background[Windows_open - 1],
                       Window_pos_X,
                       Window_pos_Y,
                       Window_width,
                       Window_height);
...
void
Window_draw_normal_bouton(word x_pos,
                          word y_pos,
                          word width,
                          word height,
                          const char* title,
                          ...
                          byte clickable)
{
  ...
  if (clickable) {
    ...
    Window_display_frame_generic(x_pos - 1,
                                 y_pos - 1,
                                 width + 2,
                                 height + 2,
                                 MC_Black,
    ...
    Window_display_frame_mono(
      x_pos - 1, y_pos - 1, width + 2, height + 2, MC_Light);
    title_color = MC_Dark;
  }
  ...
  if (undersc_letter)
    Window_rectangle(
      text_x_pos + ((undersc_letter - 1) << 3), text_y_pos + 8, 8, 1, MC_Dark);
#else
  (void)undersc_letter;
#endif
}
...
void
Window_select_normal_button(word x_pos, word y_pos, word width, word height)
{
  Window_display_frame_generic(
    x_pos, y_pos, width, height, MC_Dark, MC_Black, MC_Dark, MC_Dark, MC_Black);
  Update_window_area(x_pos, y_pos, width, height);
}

// -- Button normal désenfoncé dans la fenêtre --
void
Window_unselect_normal_button(word x_pos, word y_pos, word width, word height)
{
  Window_display_frame_out(x_pos, y_pos, width, height);
  Update_window_area(x_pos, y_pos, width, height);
}
...
void
Window_draw_palette_bouton(word x_pos, word y_pos)
{
  word color;

  for (color = 0; color <= 255; color++)
    Window_rectangle(((color >> 4) * 10) + x_pos + 6,
                     ((color & 15) * 5) + y_pos + 3,
                     5,
                     5,
                     color);
...
Window_clear_tags(void)
{
  ...
  for (x_pos = 0, window_x_pos = origin_x; x_pos < 16;
       x_pos++, window_x_pos += 10)
...
void
Tag_color_range(byte start, byte end)
{
  word origin_x;
  word origin_y;
  // word x_pos;
  ...
  for (index = 0; index <= start; index++)
    Window_rectangle(Window_palette_button_list->Pos_X + 3 +
                       ((index >> 4) * 10),
  ...
  for (y_pos = 0, window_y_pos = origin_y + 1; y_pos < 3;
       y_pos++, window_y_pos++)
  ...
  if (start != end) {
    ...
    for (index = start + 1; index < end; index++) {
      Window_rectangle(
        Window_palette_button_list->Pos_X + 3 + ((index >> 4) * 10),
        Window_palette_button_list->Pos_Y + 3 + ((index & 15) * 5),
        2,
        5,
        MC_Black);
      ...
      Pixel_in_window(
        Window_palette_button_list->Pos_X + 5 + ((index >> 4) * 10),
        Window_palette_button_list->Pos_Y + 5 + ((index & 15) * 5),
        MC_Light);
    }
  }

  Update_window_area(Window_palette_button_list->Pos_X + 3,
                     Window_palette_button_list->Pos_Y + 3,
                     12 * 16,
                     5 * 16);
}
...
void
Compute_slider_cursor_length(T_Scroller_button* button)
{
  if (button->Nb_elements > button->Nb_visibles) {
    button->Cursor_length =
      (button->Nb_visibles * (button->Length - 24)) / button->Nb_elements;
...
void
Window_draw_slider(T_Scroller_button* button)
{
  ...
  if (button->Is_horizontal) {
    ...
    if (button->Nb_elements > button->Nb_visibles) {
      int pos =
        ((button->Length - 24 - button->Cursor_length) * button->Position +
    ...
    Update_window_area(button->Pos_X, button->Pos_Y, button->Length, 11);
  } else {
    ...
    Update_window_area(button->Pos_X, button->Pos_Y, 11, button->Length);
  }
}

void
Window_draw_scroller_button(T_Scroller_button* button)
{
  if (button->Is_horizontal) {
    Window_display_frame_generic(button->Pos_X - 1,
                                 button->Pos_Y - 1,
                                 button->Length + 2,
                                 13,
                                 MC_Black,
                                 ...
                                 MC_Dark);
    Window_display_frame_mono(
      button->Pos_X + 11, button->Pos_Y - 1, button->Length - 22, 13, MC_Black);
    Window_display_frame_out(button->Pos_X, button->Pos_Y, 11, 11);
    Window_display_frame_out(
      button->Pos_X + button->Length - 11, button->Pos_Y, 11, 11);
    Print_in_window(
      button->Pos_X + 2, button->Pos_Y + 2, "\033", MC_Black, MC_Light);
    Print_in_window(button->Pos_X + button->Length - 9,
                    button->Pos_Y + 2,
                    "\032",
                    MC_Black,
                    MC_Light);
    ...
    Window_display_frame_generic(button->Pos_X - 1,
                                 ...
                                 MC_Dark);
    Window_display_frame_mono(
      button->Pos_X - 1, button->Pos_Y + 11, 13, button->Length - 22, MC_Black);
    Window_display_frame_out(button->Pos_X, button->Pos_Y, 11, 11);
    Window_display_frame_out(
      button->Pos_X, button->Pos_Y + button->Length - 11, 11, 11);
    Print_in_window(
      button->Pos_X + 2, button->Pos_Y + 2, "\030", MC_Black, MC_Light);
    Print_in_window(button->Pos_X + 2,
                    button->Pos_Y + button->Length - 9,
                    "\031",
                    MC_Black,
                    MC_Light);
  }
...
void
Window_draw_input_bouton(word x_pos, word y_pos, word width_in_characters)
{
...
void
Window_input_content(T_Special_button* button, const char* content)
{
  Print_in_window_limited(button->Pos_X + 2,
                          button->Pos_Y + 2,
                          content,
                          button->Width / 8,
                          MC_Black,
...
void
Window_input_content_unicode(T_Special_button* button, const word* content)
{
  Print_in_window_limited_unicode(button->Pos_X + 2,
                                  button->Pos_Y + 2,
                                  content,
                                  button->Width / 8,
                                  MC_Black,
...
void
Window_clear_input_button(T_Special_button* button)
{
  Window_rectangle(
    button->Pos_X + 2, button->Pos_Y + 2, (button->Width / 8) * 8, 8, MC_Light);
  Update_window_area(
    button->Pos_X + 2, button->Pos_Y + 2, button->Width / 8 * 8, 8);
}

//------ Rajout d'un bouton à la liste de ceux présents dans la fenêtre ------
// undersc_letter is 0 for no underscore, 1-indexed array index otherwise
T_Normal_button*
Window_set_normal_button(word x_pos,
                         word y_pos,
                         word width,
                         word height,
                         const char* title,
                         ...
                         word shortcut)
{
  ...
  Window_draw_normal_bouton(
    x_pos, y_pos, width, height, title, undersc_letter, clickable);
  return temp;
}
//------ Rajout d'un bouton à la liste de ceux présents dans la fenêtre ------
// undersc_letter is 0 for no underscore, 1-indexed array index otherwise
T_Normal_button*
Window_set_repeatable_button(word x_pos,
                             word y_pos,
                             word width,
                             word height,
                             const char* title,
...
T_Palette_button*
Window_set_palette_button(word x_pos, word y_pos)
{
...
T_Scroller_button*
Window_set_scroller_button(word x_pos,
                           word y_pos,
                           word height,
                           word nb_elements,
...
T_Scroller_button*
Window_set_horizontal_scroller_button(word x_pos,
                                      word y_pos,
                                      word width,
                                      word nb_elements,
                                      word nb_elements_visible,
...
T_Special_button*
Window_set_special_button(word x_pos,
                          word y_pos,
                          word width,
                          word height,
                          word shortcut)
...
T_Special_button*
Window_set_input_button_s(word x_pos,
                          word y_pos,
                          word width_in_characters,
                          word shortcut)
{
  T_Special_button* temp;
  temp = Window_set_special_button(
    x_pos, y_pos, (width_in_characters << 3) + 3, 11, shortcut);
  Window_draw_input_bouton(x_pos, y_pos, width_in_characters);
  return temp;
}

T_Special_button*
Window_set_input_button(word x_pos, word y_pos, word width_in_characters)
{
  return Window_set_input_button_s(x_pos, y_pos, width_in_characters, 0);
}

T_Dropdown_button*
Window_set_dropdown_button(word x_pos,
                           word y_pos,
                           word width,
                           word height,
                           word dropdown_width,
                           ...
                           byte bottom_up)
{
  ...
  if (label && label[0])
    Print_in_window(temp->Pos_X + 2,
                    temp->Pos_Y + (temp->Height - 7) / 2,
                    label,
                    MC_Black,
                    MC_Light);
  if (display_arrow)
    Window_display_icon_sprite(temp->Pos_X + temp->Width - 10,
                               temp->Pos_Y + (temp->Height - 7) / 2,
                               ICON_DROPDOWN);
...
void
Window_dropdown_add_item(T_Dropdown_button* dropdown,
                         word btn_number,
                         const char* label)
{
...
void
Window_dropdown_clear_items(T_Dropdown_button* dropdown)
{
...
T_List_button*
Window_set_list_button(T_Special_button* entry_button,
...
void
Window_redraw_list(T_List_button* list)
{
  int i;

  for (i =
         Min(list->Scroller->Nb_visibles - 1, list->Scroller->Nb_elements - 1);
       i >= 0;
       i--) {
    list->Draw_list_item(list->Entry_button->Pos_X,
  ...
  if (i > 0) {
    byte color;
    color = list->Color_index == 0
              ? MC_Black
              : (list->Color_index == 1
                   ? MC_Dark
                   : (list->Color_index == 2 ? MC_Light : MC_White));

    Window_rectangle(list->Entry_button->Pos_X,
                     list->Entry_button->Pos_Y +
                       list->Scroller->Nb_elements * 8,
 ...
 */
void
Open_popup(word x_pos, word y_pos, word width, word height)
{
  Windows_open++;

  if (height * Menu_factor_Y > Screen_height)
    height = Screen_height / Menu_factor_Y;
  if (y_pos + height * Menu_factor_Y >
      Screen_height) // fix dropdown that would get bellow the screen
...
byte
Window_click_in_rectangle(short start_x,
                          short start_y,
                          short end_x,
                          short end_y)
{
  ...
  return ((x_pos >= start_x) && (y_pos >= start_y) && (x_pos <= end_x) &&
          (y_pos <= end_y));
}
...
short
Wait_click_in_palette(T_Palette_button* button)
{
  ...
  for (;;) {
    ...
    if (Mouse_K == LEFT_SIDE) {
      if (Window_click_in_rectangle(start_x, start_y, end_x, end_y)) {
        Hide_cursor();
        selected_color =
          (((Mouse_X - Window_pos_X) / Menu_factor_X) - (button->Pos_X + 2)) /
            10 * 16 +
          (((Mouse_Y - Window_pos_Y) / Menu_factor_Y) - (button->Pos_Y + 3)) /
            5;
        Cursor_shape = CURSOR_SHAPE_ARROW;
...
void
Get_color_behind_window(byte* color, byte* click)
{
  short old_x = -1;
  short old_y = -1;
  short index;
  ...
  Save_background(
    &buffer, Window_pos_X, Window_pos_Y, Window_width, Window_height);
  a = Menu_Y;
  Menu_Y = Menu_Y_before_window;
  b = Menu_is_visible;
  Menu_is_visible = Menu_is_visible_before_window;
  ...
  do {
    ...
    if ((Mouse_X != old_x) || (Mouse_Y != old_y)) {
      ...
      if (a != c) {
        ...
        if (Menu_is_visible_before_window) {
          d = snprintf(
            str, sizeof(str), "%d", a); // position of the space to color
          index = snprintf(str + d,
                           sizeof(str) - d,
                           "   (%d,%d,%d)",
                           Main.palette[a].R,
                           Main.palette[a].G,
          ...
          Print_general(
            (26 +
             ((d + strlen(Buttons_Pool[BUTTON_CHOOSE_COL].Tooltip)) << 3)) *
              Menu_factor_X,
            Menu_status_Y,
            " ",
            0,
...
Move_window(short dx, short dy)
{
  ...
  if ((new_x != Window_pos_X) || (new_y != Window_pos_Y)) {
    ...
    Update_rect((new_x > Window_pos_X) ? Window_pos_X : new_x,
                (new_y > Window_pos_Y) ? Window_pos_Y : new_y,
                ((new_x > Window_pos_X) ? (new_x - Window_pos_X)
                                        : (Window_pos_X - new_x)) +
                  Window_width * Menu_factor_X,
                ((new_y > Window_pos_Y) ? (new_y - Window_pos_Y)
                                        : (Window_pos_Y - new_y)) +
                  Window_height * Menu_factor_Y);
    Window_pos_X = new_x;
    Window_pos_Y = new_y;
...
T_Dropdown_choice*
Dropdown_activate(T_Dropdown_button* button, short off_x, short off_y)
{
  short nb_choices;
  short choice_index;
...
#define SHADOW_BOTTOM 4
  ...
  Open_popup(off_x + (button->Pos_X) * Menu_factor_X,
             off_y + (button->Pos_Y +
                      (button->Bottom_up ? -box_height : button->Height)) *
                       Menu_factor_Y,
  ...
  if (SHADOW_BOTTOM) {
    Window_rectangle(
      SHADOW_RIGHT, box_height, box_width, SHADOW_BOTTOM, MC_Black);
    Window_rectangle(0, box_height, SHADOW_RIGHT, 1, MC_Black);
  }
  if (SHADOW_RIGHT) {
    Window_rectangle(box_width,
                     ...
                     box_height - SHADOW_BOTTOM,
                     MC_Black);
    Window_rectangle(box_width, 1, 1, SHADOW_BOTTOM, MC_Black);
  }
  ...
  while (1) {
    ...
    y = 2;
    // Affichage des items
    for (item = button->First_item, choice_index = 0; item != NULL;
         item = item->Next, choice_index++) {
      byte color_1;
      byte color_2;
  ...
  if (selected_index >= 0 && selected_index < nb_choices) {
    for (item = button->First_item; selected_index;
         item = item->Next, selected_index--)
      ;
    return item;
  }
  return NULL;
...
short
Window_dropdown_on_click(T_Dropdown_button* button)
{
  T_Dropdown_choice* item;
  ...
  if (button->Display_choice) {
    ...
    Window_rectangle(button->Pos_X + 2,
                     button->Pos_Y + (button->Height - 7) / 2,
                     text_length * 8,
                     8,
                     MC_Light);
    Print_in_window_limited(button->Pos_X + 2,
...
short
Window_normal_button_onclick(word x_pos,
                             word y_pos,
                             word width,
                             word height,
                             short btn_number)
{
  while (1) {
    ...
    Display_cursor();
    while (Window_click_in_rectangle(
      x_pos, y_pos, x_pos + width - 1, y_pos + height - 1)) {
      Get_input(20);
      if (!Mouse_K) {
        Hide_cursor();
...
// ---
short
Window_get_clicked_button(void)
{
  T_Normal_button* temp1;
  T_Palette_button* temp2;
  T_Scroller_button* temp3;
  ...
  for (temp1 = Window_normal_button_list; temp1; temp1 = temp1->Next) {
    if ((Input_sticky_control == 0 || Input_sticky_control == temp1->Number) &&
        Window_click_in_rectangle(temp1->Pos_X,
                                  temp1->Pos_Y,
                                  temp1->Pos_X + temp1->Width - 1,
                                  temp1->Pos_Y + temp1->Height - 1)) {
      Input_sticky_control = temp1->Number;
      if (temp1->Repeatable) {
        Hide_cursor();
        Window_select_normal_button(
          temp1->Pos_X, temp1->Pos_Y, temp1->Width, temp1->Height);
        Display_cursor();
        Delay_with_active_mouse((Mouse_K == 1)
                                  ? Config.Delay_left_click_on_slider
                                  : Config.Delay_right_click_on_slider);
        Hide_cursor();
        Window_unselect_normal_button(
          temp1->Pos_X, temp1->Pos_Y, temp1->Width, temp1->Height);
        Display_cursor();
      ...
      return Window_normal_button_onclick(
        temp1->Pos_X, temp1->Pos_Y, temp1->Width, temp1->Height, temp1->Number);
    }
  }
  ...
  for (temp2 = Window_palette_button_list; temp2; temp2 = temp2->Next) {
    if ((Input_sticky_control == 0 || Input_sticky_control == temp2->Number) &&
        Window_click_in_rectangle(temp2->Pos_X + 5,
                                  temp2->Pos_Y + 3,
                                  temp2->Pos_X + 160,
                                  temp2->Pos_Y + 82)) {
      ...
      Window_attribute2 =
        (((Mouse_X - Window_pos_X) / Menu_factor_X) - (temp2->Pos_X + 2)) / 10 *
          16 +
  ...
  for (temp3 = Window_scroller_button_list; temp3; temp3 = temp3->Next) {
    // Button Up arrow
    if ((Input_sticky_control == 0 ||
         Input_sticky_control == (temp3->Number | 1024)) &&
        Window_click_in_rectangle(
          temp3->Pos_X, temp3->Pos_Y, temp3->Pos_X + 10, temp3->Pos_Y + 10)) {
      Input_sticky_control = temp3->Number | 1024;
      Hide_cursor();
    ...
    if ((Input_sticky_control == 0 ||
        ...
        ((temp3->Is_horizontal &&
          Window_click_in_rectangle(temp3->Pos_X + temp3->Length - 11,
                                    temp3->Pos_Y,
                                    temp3->Pos_X + temp3->Length - 1,
                                    temp3->Pos_Y + 10)) ||
         (!temp3->Is_horizontal &&
          Window_click_in_rectangle(temp3->Pos_X,
                                    temp3->Pos_Y + temp3->Length - 11,
                                    temp3->Pos_X + 10,
                                    temp3->Pos_Y + temp3->Length - 1)))) {
      Input_sticky_control = temp3->Number | 2048;
      Hide_cursor();
      if (temp3->Is_horizontal)
        Window_select_normal_button(
          temp3->Pos_X + temp3->Length - 11, temp3->Pos_Y, 11, 11);
      else
        Window_select_normal_button(
          temp3->Pos_X, temp3->Pos_Y + temp3->Length - 11, 11, 11);
    ...
    // Middle slider
    if ((Input_sticky_control == temp3->Number) ||
        (Input_sticky_control == 0 &&
         ((!temp3->Is_horizontal &&
           Window_click_in_rectangle(temp3->Pos_X,
                                     temp3->Pos_Y + 12,
                                     temp3->Pos_X + 10,
                                     temp3->Pos_Y + temp3->Length - 13)) ||
          (temp3->Is_horizontal &&
           Window_click_in_rectangle(temp3->Pos_X + 12,
                                     temp3->Pos_Y,
                                     temp3->Pos_X + temp3->Length - 13,
                                     temp3->Pos_Y + 10))))) {
      ...
      if (temp3->Nb_elements > temp3->Nb_visibles) {
        ...
        if (temp3->Is_horizontal)
          mouse_pos =
            (Mouse_X - Window_pos_X) / Menu_factor_X - (temp3->Pos_X + 12);
        else
          mouse_pos =
            (Mouse_Y - Window_pos_Y) / Menu_factor_Y - (temp3->Pos_Y + 12);
        ...
        origin = (temp3->Nb_visibles - 1) * (temp3->Length - 24) /
                 temp3->Nb_elements / 2;
        Window_attribute2 =
          (mouse_pos - origin) *
          (temp3->Nb_elements - (temp3->Cursor_length > 1 ? 0 : 1)) /
          (temp3->Length - 24 - 1);
  ...
  for (temp4 = Window_special_button_list; temp4; temp4 = temp4->Next) {
    if ((Input_sticky_control == 0 || Input_sticky_control == temp4->Number) &&
        Window_click_in_rectangle(temp4->Pos_X,
                                  temp4->Pos_Y,
                                  temp4->Pos_X + temp4->Width - 1,
                                  temp4->Pos_Y + temp4->Height - 1)) {
  ...
  for (temp5 = Window_dropdown_button_list; temp5; temp5 = temp5->Next) {
    if ((Input_sticky_control == 0 || Input_sticky_control == temp5->Number) &&
        Window_click_in_rectangle(temp5->Pos_X,
                                  temp5->Pos_Y,
                                  temp5->Pos_X + temp5->Width - 1,
                                  temp5->Pos_Y + temp5->Height - 1)) {
      Input_sticky_control = temp5->Number;
      if (Mouse_K & temp5->Active_button)
        return Window_dropdown_on_click(temp5);
      else {
        Window_attribute2 = -1;
        return Window_normal_button_onclick(temp5->Pos_X,
                                            temp5->Pos_Y,
                                            temp5->Width,
                                            temp5->Height,
                                            temp5->Number);
...
short
Window_get_button_shortcut(void)
{
  ...
  while (temp != NULL) {
    if (temp->Shortcut == Key) {
      Hide_cursor();
      Window_select_normal_button(
        temp->Pos_X, temp->Pos_Y, temp->Width, temp->Height);
      Display_cursor();
...
Window_clicked_button(void)
{
  ...
  if (Mouse_K) {
    ...
    if (!Input_sticky_control && Window_draggable &&
        Mouse_Y < Window_pos_Y + (12 * Menu_factor_Y)) {
      ...
      for (list = Window_list_button_list; list != NULL; list = list->Next) {
        if (list->Entry_button->Number == clicked_button) {
          ...
          short clicked_line;
          clicked_line = (((Mouse_Y - Window_pos_Y) / Menu_factor_Y) -
                          list->Entry_button->Pos_Y) >>
                         3;
          ...
          if (clicked_line == list->Cursor_position) {
            // Double click check
            if (old_mouse_k == 0 && last_list_number == list->Number &&
                time_now - time_last_click < Config.Double_click_speed) {
              time_last_click = time_now;
              Input_sticky_control = 0;
              // Store the selected value as attribute2
              Window_attribute2 = list->List_start + list->Cursor_position;
              // Return the control ID of the "special button" that covers the
              // list.
  ...
  if (Key) {
    ...
    for (list = Window_list_button_list; list != NULL; list = list->Next) {
      ...
      if (1) {
        ...
        if (Key == KEY_DOWN && (list->Cursor_position + list->List_start) <
                                 (list->Scroller->Nb_elements - 1)) {
          Key = 0;
          Hide_cursor();
          list->Cursor_position++;
          if (list->Cursor_position > (list->Scroller->Nb_visibles - 1)) {
            list->List_start = list->List_start + list->Cursor_position -
                               (list->Scroller->Nb_visibles - 1);
        ...
        if (Key == KEY_HOME &&
            (list->Cursor_position != 0 || list->List_start != 0)) {
          Key = 0;
          Hide_cursor();
          list->Cursor_position = 0;
          list->List_start = 0;
        ...
        if (Key == KEY_END && (list->Cursor_position + list->List_start) <
                                (list->Scroller->Nb_elements - 1)) {
          Key = 0;
          Hide_cursor();
          list->Cursor_position =
            (list->Scroller->Nb_elements - 1) - list->List_start;
          if (list->Cursor_position > (list->Scroller->Nb_visibles - 1)) {
            list->List_start = list->List_start + list->Cursor_position -
                               (list->Scroller->Nb_visibles - 1);
            list->Cursor_position = (list->Scroller->Nb_visibles - 1);
        ...
        if (Key == KEY_PAGEDOWN && (list->Cursor_position + list->List_start) <
                                     (list->Scroller->Nb_elements - 1)) {
          Key = 0;
          Hide_cursor();
          if (list->Scroller->Nb_elements < list->Scroller->Nb_visibles) {
            ...
            if (list->List_start + list->Scroller->Nb_visibles >
                list->Scroller->Nb_elements) {
              list->List_start =
                list->Scroller->Nb_elements - list->Scroller->Nb_visibles;
            }
            // Mise à jour du scroller
            list->Scroller->Position = list->List_start;
            Window_draw_slider(list->Scroller);
        ...
        if (Key == KEY_PAGEUP &&
            (list->Cursor_position + list->List_start) > 0) {
          Key = 0;
          Hide_cursor();
          if (list->Cursor_position != 0) {
            list->Cursor_position = 0;
...
void
Remap_window_backgrounds(const byte* conversion_table, int Min_Y, int Max_Y)
{
  int window_index;
  byte* EDI;
  int dx, cx;
  ...
  for (window_index = 0; window_index < Windows_open; window_index++) {
    ...
    for (dx = 0; dx < Window_stack[window_index].Height * Menu_factor_Y; dx++) {
      ...
      else { // Pour chaque pixel
        for (cx =
               Window_stack[window_index].Width * Menu_factor_X * Pixel_width;
             cx > 0;
             cx--) {
...
void
Remap_UI_in_window_backgrounds(const byte* conversion_table)
{
  int i, j;
  int pos_y;
  ...
  for (i = 0; i < Windows_open; i++) {
    ...
    for (pos_y = Window_stack[i].Pos_Y;
         pos_y <
         (Window_stack[i].Pos_Y + Window_stack[i].Height * Menu_factor_Y);
         pos_y++) {
      ...
      if (Menu_is_visible_before_window && pos_y >= Menu_Y_before_window) {
        ...
        for (j = 0; j < i; j++) {
          ...
          if (pos_y < Window_stack[j].Pos_Y ||
              pos_y >= (Window_stack[j].Pos_Y +
                        Window_stack[j].Height * Menu_factor_Y))
            continue; // this window doesn't occupy this screen row
          if (min_x > Window_stack[j].Pos_X)
            min_x = Window_stack[j].Pos_X;
          if (max_x <
              (Window_stack[j].Pos_X + Window_stack[j].Width * Menu_factor_X))
            max_x =
              Window_stack[j].Pos_X + Window_stack[j].Width * Menu_factor_X;
        }
...
void
Delay_with_active_mouse(int speed)
{
  dword end;
...
void
Set_bar_visibility(word bar, int visible, int with_redraw)
{
