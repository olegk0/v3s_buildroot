...
static const MDFNSetting_EnumList SDriver_List[] = {
	...
	{ "wasapish", -1, "WASAPI(Shared Mode)",
	  gettext_noop("The default when it's available(running on Microsoft Windows Vista and newer).") },

	{ "dsound", -1, "DirectSound", gettext_noop("The default for Microsoft Windows XP and older.") },

	{ "wasapi", -1, "WASAPI(Exclusive Mode)",
	  gettext_noop(
	...
	{ "sdl", -1, "Simple Directmedia Layer",
	  gettext_noop(
...
static const MDFNSetting DriverSettings[] = {
	{ "input.joystick.global_focus", MDFNSF_NOFLAGS,
	  gettext_noop("Update physical joystick(s) internal state in Mednafen even when Mednafen lacks OS focus."), NULL,
	  MDFNST_BOOL, "1" },
	{ "input.joystick.axis_threshold", MDFNSF_NOFLAGS, gettext_noop("Analog axis binary press detection threshold."),
	  gettext_noop("Threshold for detecting a digital-like \"button\" press on analog axis, in percent."), MDFNST_FLOAT, "75",
	  "0", "100" },
	{ "input.autofirefreq", MDFNSF_NOFLAGS, gettext_noop("Auto-fire frequency."),
	  gettext_noop("Auto-fire frequency = GameSystemFrameRateHz / (value + 1)"), MDFNST_UINT, "3", "0", "1000" },
	{ "input.ckdelay", MDFNSF_NOFLAGS, gettext_noop("Dangerous key action delay."),
	  gettext_noop(
	...
	{ "netplay.console.font", MDFNSF_NOFLAGS, gettext_noop("Font for netplay chat console."), NULL, MDFNST_ENUM, "9x18", NULL,
	  NULL, NULL, NULL, FontSize_List },
	{ "netplay.console.scale", MDFNSF_NOFLAGS, gettext_noop("Netplay chat console text scale factor."),
	  gettext_noop("A value of 0 enables auto-scaling."), MDFNST_UINT, "1", "0", "16" },
	{ "netplay.console.lines", MDFNSF_NOFLAGS, gettext_noop("Height of chat console, in lines."), NULL, MDFNST_UINT, "5", "5",
	  "64" },
	...
	{ "autosave", MDFNSF_NOFLAGS, gettext_noop("Automatically load/save state on game load/close."),
	  gettext_noop("Automatically save and load save states when a game is closed or loaded, respectively."), MDFNST_BOOL,
	  "0" },
	{ "sound.driver", MDFNSF_NOFLAGS, gettext_noop("Select sound driver."),
	  gettext_noop(
	  ...
	  MDFNST_ENUM, "default", NULL, NULL, NULL, NULL, SDriver_List },
	{ "sound.device", MDFNSF_NOFLAGS, gettext_noop("Select sound output device."),
	  gettext_noop(
	  ...
	  MDFNST_STRING, "default", NULL, NULL },
	{ "sound.volume", MDFNSF_NOFLAGS, gettext_noop("Sound volume level, in percent."),
	  gettext_noop("Setting this volume control higher than the default of \"100\" may severely distort the sound."),
	  MDFNST_UINT, "100", "0", "150" },
	{ "sound", MDFNSF_NOFLAGS, gettext_noop("Enable sound output."), NULL, MDFNST_BOOL, "1" },
	{ "sound.period_time", MDFNSF_NOFLAGS, gettext_noop("Desired period size in microseconds(μs)."),
	  gettext_noop(
	  ...
	  MDFNST_UINT, "0", "0", "100000" },
	{ "sound.buffer_time", MDFNSF_NOFLAGS, gettext_noop("Desired buffer size in milliseconds(ms)."),
	  gettext_noop("The default value of 0 enables automatic buffer size selection."), MDFNST_UINT, "0", "0", "1000" },
	{ "sound.rate", MDFNSF_NOFLAGS, gettext_noop("Specifies the sound playback rate, in sound frames per second(\"Hz\")."),
	  NULL, MDFNST_UINT, "48000", "22050", "192000" },

#ifdef WANT_DEBUGGER
	{ "debugger.autostepmode", MDFNSF_NOFLAGS,
	  gettext_noop("Automatically go into the debugger's step mode after a game is loaded."), NULL, MDFNST_BOOL, "0" },
#endif

	{ "osd.message_display_time", MDFNSF_NOFLAGS,
	  gettext_noop("Length of time, in milliseconds, to display internal status and error messages"),
	  gettext_noop(
	...
	{ "fps.position", MDFNSF_NOFLAGS, gettext_noop("FPS display position."), NULL, MDFNST_ENUM, "upper_left", NULL, NULL, NULL,
	  NULL, FPSPos_List },
	{ "fps.scale", MDFNSF_NOFLAGS, gettext_noop("FPS display scale factor."),
	  gettext_noop("A value of 0 enables auto-scaling."), MDFNST_UINT, "1", "0", "32" },
	{ "fps.font", MDFNSF_NOFLAGS, gettext_noop("FPS display font."), NULL, MDFNST_ENUM, "5x7", NULL, NULL, NULL, NULL,
	  FontSize_List },
	{ "fps.textcolor", MDFNSF_NOFLAGS, gettext_noop("FPS display text color."), gettext_noop("0xAARRGGBB"), MDFNST_UINT,
	  "0xFFFFFFFF", "0x00000000", "0xFFFFFFFF" },
	{ "fps.bgcolor", MDFNSF_NOFLAGS, gettext_noop("FPS display background color."), gettext_noop("0xAARRGGBB"), MDFNST_UINT,
	  "0x80000000", "0x00000000", "0xFFFFFFFF" },

	{ "srwautoenable", MDFNSF_SUPPRESS_DOC,
	  gettext_noop(
		  "DO NOT USE UNLESS YOU'RE A SPACE GOAT" /*"Automatically enable state rewinding functionality on game load."*/),
	  gettext_noop(
		  "Use this setting with caution, as save state rewinding can have widely variable memory and CPU usage requirements among different games and different emulated systems."),
	  MDFNST_BOOL, "0" },

	{ "affinity.emu", MDFNSF_NOFLAGS, gettext_noop("Main emulation thread CPU affinity mask."),
	  gettext_noop("Set to 0 to disable changing affinity."), MDFNST_UINT, "0", "0x0000000000000000", "0xFFFFFFFFFFFFFFFF" },
	{ "affinity.video", MDFNSF_NOFLAGS, gettext_noop("Video blitting thread CPU affinity mask."),
	  gettext_noop("Set to 0 to disable changing affinity."), MDFNST_UINT, "0", "0x0000000000000000", "0xFFFFFFFFFFFFFFFF" },

	{ NULL }
};

void AddSystemSetting(const char *system_name, const char *name, const char *description, const char *description_extra,
		      MDFNSettingType type, const char *default_value, const char *minimum, const char *maximum,
...
void MakeDebugSettings(void)
{
	...
	for (unsigned int i = 0; i < MDFNSystems.size(); i++) {
		...
		AddSystemSetting(
			sysname, "debugger.disfontsize", gettext_noop("Disassembly font size."),
			gettext_noop(
				"Note: Setting the font size to larger than the default may cause text overlap in the debugger."),
			MDFNST_ENUM, "5x7", NULL, NULL, NULL, NULL, FontSize_List, extra_flags);
		AddSystemSetting(sysname, "debugger.memcharenc",
				 gettext_noop("Character encoding for the debugger's memory editor."), NULL, MDFNST_STRING,
				 dbg->DefaultCharEnc, NULL, NULL, NULL, NULL, NULL, extra_flags);
	}
#endif
}
...
{
	decltype(GetConsoleWindow) *p_GetConsoleWindow =
		(decltype(GetConsoleWindow) *)GetProcAddress(GetModuleHandleA("kernel32.dll"), "GetConsoleWindow");
...
static bool DoArgs(int argc, char *argv[], char **filename)
{
	...
	ARGPSTRUCT MDFNArgs[] = {
		...
		{ "remote", _("Enable remote mode with the specified stdout key(EXPERIMENTAL AND INCOMPLETE)."), 0, &dummy_remote,
		  SUBSTYPE_STRING_ALLOC },
		...
		{ "which_medium", _("Start with specified disk/CD(numbered from 0) inserted."), 0, &which_medium,
		  SUBSTYPE_INTEGER },

		{ "force_module", _("Force usage of specified emulation module."), 0, &force_module_arg, SUBSTYPE_STRING_ALLOC },

		{ "soundrecord", _("Record sound output to the specified filename in the MS WAV format."), 0, &soundrecfn,
		  SUBSTYPE_STRING_ALLOC },
		{ "qtrecord", _("Record video and audio output to the specified filename in the QuickTime format."), 0, &qtrecfn,
		  SUBSTYPE_STRING_ALLOC }, // TODOC: Video recording done without filtering applied.
...
static int LoadGame(const char *force_module, const char *path)
{
	...
	if (StatePCTest) {
		for (unsigned i = 0; i < 2; i++) {
			...
			if (!(state0.map_size() == state1.map_size() &&
			      !memcmp(state0.map() + 32, state1.map() + 32, state1.map_size() - 32))) {
...
static int GameLoop(void *arg)
{
	 ...
	 */
		...
		if (MDFN_UNLIKELY(StateRCTest)) {
			...
			if (!(state1.map_size() == state2.map_size() &&
			      !memcmp(state1.map() + 32, state2.map() + 32, state1.map_size() - 32))) {
				FileStream sd0("/tmp/sdump0", FileStream::MODE_WRITE);
				FileStream sd1("/tmp/sdump1", FileStream::MODE_WRITE);
		...
		if (MDFN_UNLIKELY(StateSLSTest)) {
			...
			if (!(orig_state.map_size() == new_state.map_size() &&
			      !memcmp(orig_state.map() + 32, new_state.map() + 32, orig_state.map_size() - 32))) {
				FileStream sd0("/tmp/sdump0", FileStream::MODE_WRITE);
				FileStream sd1("/tmp/sdump1", FileStream::MODE_WRITE);
...
{
	SendCEvent(CEVT_SET_WMINPUTBEHAVIOR, nullptr, nullptr,
		   (CursorNeeded << 0) | (MouseAbsNeeded << 1) | (MouseRelNeeded << 2) | (GrabNeeded << 3));
}
...
{
	MDFN_printf(_("Compiled against SDL %u.%u.%u\n"), SDL_MAJOR_VERSION, SDL_MINOR_VERSION, SDL_PATCHLEVEL);
}
...
void PrintLIBICONVVersion(void)
{
	...
	MDFN_printf(_("Compiled against libiconv %u.%u, running with libiconv %u.%u\n"), _LIBICONV_VERSION & 0xFF,
		    _LIBICONV_VERSION >> 8, _libiconv_version & 0xFF, _libiconv_version >> 8);
#endif
}
			...
			for (const MDFNCS &s : *cs) {
				if (s.desc.flags & MDFNSF_CAT_PATH) {
					...
					if (s_value_len > 0) {
						...
						if ((req = MultiByteToWideChar(CP_ACP, 0, s.value, s_value_len, NULL, 0)) >
						    0) // not CP_THREAD_ACP
						{
							std::unique_ptr<char16_t[]> ws(new char16_t[req]);

							if (MultiByteToWideChar(CP_ACP, 0, s.value, s_value_len,
										(wchar_t *)ws.get(), req) == req)
								MDFNI_SetSetting(s.desc.name, UTF16_to_UTF8(ws.get(), req));
							else
								throw MDFN_Error(
									0, _("Error converting value of setting \"%s\" to UTF-8."),
									s.desc.name);
						} else
							throw MDFN_Error(0, _("Error converting value of setting \"%s\" to UTF-8."),
									 s.desc.name);
					}
				}
			}
...
int main(int argc, char *argv[])
{
	...
	if (!MSW_GetArgcArgv(&argc, &argv)) {
		if (!SuppressErrorPopups)
			MessageBoxA(NULL, "Error getting/allocating arguments.", "Mednafen Startup Error",
...
//
	...
	if (!(GetVersion() & 0x80000000)) {
		const char *errorstr = _(
			"This special build of Mednafen is intended for use on Windows 98, Windows 98SE, and Windows Me.  It will perform suboptimally on NT-based versions of Windows.  Download a regular Windows build from https://mednafen.github.io/releases/");

		if (!SuppressErrorPopups)
			MessageBoxA(NULL, errorstr, "Mednafen Windows Version Error",
				    MB_OK | MB_ICONSTOP | MB_TASKMODAL | MB_SETFOREGROUND | MB_TOPMOST);
		...
		try {
			...
			if (e.GetErrno() == EWOULDBLOCK) // Fragile, FIXME with proper class-specific error types.
			{
				...
				if (env_aw && atoi(env_aw) != 0) {
					MDFN_printf(_(
						"Error, but proceeding anyway per environment variable \"MEDNAFEN_ALLOWMULTI\".\n"));
				} else {
					MDFN_Notify(
						MDFN_NOTICE_ERROR,
						_("Multiple instances of Mednafen using the same base directory should not run simultaneously, otherwise settings file changes may be lost, along with other similar problems.  If you understand the risks, and want to anyway, run Mednafen with environment variable \"MEDNAFEN_ALLOWMULTI\" set to \"1\" to bypass this check."));
					return -1;
				}
			} else {
 ...
 needie = argv[zgi];
	...
	try {
		if (LoadGame(force_module_arg, needie)) {
			...
			FPS_Init(MDFN_GetSettingUI("fps.position"), MDFN_GetSettingUI("fps.scale"), MDFN_GetSettingUI("fps.font"),
				 MDFN_GetSettingUI("fps.textcolor"), MDFN_GetSettingUI("fps.bgcolor"));
		...
		try {
			if (FatalVideoError > 0) {
...

				{
					...
					if (vtr >= 0) {
						BlitScreen(SoftFB[vtr].surface.get(), &SoftFB[vtr].rect, SoftFB[vtr].lw.get(),
							   VTRotated, SoftFB[vtr].field, VTSSnapshot);
