...
void Init(void)
{
	...
	SDL_EnableKeyRepeat(0, SDL_DEFAULT_REPEAT_INTERVAL); //SDL_DEFAULT_REPEAT_DELAY
}
...
void Event(const SDL_Event *event)
{
	switch (event->type) {
	case SDL_KEYDOWN:
		//if (event->key.repeat) //no repeater SDL_EnableKeyRepeat
		//	break;
		//printf("scancode:%d\n", event->key.keysym.scancode);
		if (MDFN_UNLIKELY(config_pending_bc.DeviceType == BUTTC_NONE)) {
			//LALT=56 RALT=100 LSHIFT=42 RSHIFT=54  LCTRL=29 RCTRL=97
			if (!config_isck ||
			    (event->key.keysym.scancode != MKK(LALT) && event->key.keysym.scancode != MKK(RALT) &&
