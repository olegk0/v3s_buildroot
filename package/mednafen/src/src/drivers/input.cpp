...
static INLINE int32 DTestAnalogButton(const std::vector<ButtConfig> &bc, const bool bypass_key_masking)
{
	...
	for (auto const &bce : bc) {
		switch (bce.DeviceType) {
		case BUTTC_KEYBOARD:
			accum += KBMan::TestAnalogButton(bce, bypass_key_masking);
			break;
		case BUTTC_JOYSTICK:
			accum += JoystickManager::TestAnalogButton(bce);
			break;
		case BUTTC_MOUSE:
			accum += MouseMan::TestAnalogButton(bce);
			break;
		}
	}
...
static INLINE uint32 DTestAxis(const std::vector<ButtConfig> &neg_bc, const std::vector<ButtConfig> &pos_bc,
			       const bool bypass_key_masking, const float dev_axis_scale)
{
	int32 accum = 0;
...
static int64 DTestAxisRel(const std::vector<ButtConfig> &neg_bc, const std::vector<ButtConfig> &pos_bc,
			  const bool bypass_key_masking)
{
	int64 accum = 0;
	...
	for (auto const &bce : neg_bc) {
		switch (bce.DeviceType) {
		case BUTTC_KEYBOARD:
			accum -= KBMan::TestAxisRel(bce, bypass_key_masking);
			break;
		case BUTTC_JOYSTICK:
			accum -= JoystickManager::TestAxisRel(bce);
			break;
		case BUTTC_MOUSE:
			accum -= MouseMan::TestAxisRel(bce);
			break;
		}
	}
...
static float DTestPointer(const std::vector<ButtConfig> &bc, const bool bypass_key_masking, const bool axis_hint)
{
	if (MDFN_LIKELY(bc.size())) {
		...
		case BUTTC_JOYSTICK:
			...
			return Video_PtoV_J(JoystickManager::TestAnalogButton(bce), axis_hint,
					    bce.ButtonNum & JoystickManager::JOY_BN_GUN_TRANSLATE);
...
static std::string BCGToString(const std::vector<ButtConfig> &bcg)
{
	...
	for (size_t i = 0; i < bcg.size(); i++) {
		...
		if (!MDFN_de64msb(&bc.DeviceID[0]) && !MDFN_de64msb(&bc.DeviceID[8]))
			...
			trio_snprintf(devidstr, sizeof(devidstr), "0x%016llx%016llx",
				      (unsigned long long)MDFN_de64msb(&bc.DeviceID[0]),
				      (unsigned long long)MDFN_de64msb(&bc.DeviceID[8]));
		...
		if (bc.DeviceType == BUTTC_KEYBOARD)
			trio_snprintf(tmp, sizeof(tmp), "keyboard %s %s%s", devidstr, KBMan::BNToString(bc.ButtonNum).c_str(),
				      scalestr);
		else if (bc.DeviceType == BUTTC_JOYSTICK)
			trio_snprintf(tmp, sizeof(tmp), "joystick %s %s%s", devidstr,
				      JoystickManager::BNToString(bc.ButtonNum).c_str(), scalestr);
		else if (bc.DeviceType == BUTTC_MOUSE)
			trio_snprintf(tmp, sizeof(tmp), "mouse %s %s%s", devidstr, MouseMan::BNToString(bc.ButtonNum).c_str(),
				      scalestr);
...
//
//
static bool StringToBCG(std::vector<ButtConfig> *bcg, const char *s, const char *defs = "",
			const bool abs_pointer_axis_thing = false)
{
	if (bcg)
		bcg->clear();
				...
				if (!MDFN_strazicmp(device_name, "keyboard"))
					...
					if (!MDFN_strazicmp(device_name, "joystick")) {
						...
						if (trio_sscanf(extra, "%016llx %08x", &devid, &bn) == 2) {
							...
							bc.DeviceNum = JoystickManager::GetIndexByUniqueID(
								bc.DeviceID, bc.Flags & BUTTC_FLAG_09XDEVID);
							if (JoystickManager::GetUniqueIDByIndex(
								    bc.DeviceNum, &bc.DeviceID)) // Not perfect, but eh...
								bc.Flags &= ~BUTTC_FLAG_09XDEVID;

							if (!JoystickManager::Translate09xBN(bn, &bc.ButtonNum,
											     abs_pointer_axis_thing))
								return false;
						} else
							return false;
						...
						if (trio_sscanf(extra, "%016llx %08x", &dummy_devid, &bn) == 2 ||
						    trio_sscanf(extra, "%d", &bn) == 1) {
		...
		if (bcg) {
			...
			if (any_kb) {
				...
				for (auto const &bc : defbc) {
					...
					if (bc.DeviceType == BUTTC_KEYBOARD ||
					    (bc.DeviceType == BUTTC_MOUSE && need_mouse_defaults)) {
						bcg->push_back(bc);
					}
				}
			}
	...
	do {
		...
		if (agc != INT_MAX) {
			...
			if (len < (2 + 1) || start_s[0] != '0' || start_s[1] != 'x' ||
			    !StringToDeviceID(start_s + 2, len - 2, &bc.DeviceID)) {
...
//
static char *build_string(const char *s0, const char *s1, const char *s2 = "", const char *s3 = "", const char *s4 = "",
...
static void BuildPortInfo(MDFNGI *gi, const unsigned int port)
{
	...
	free(port_device_name);
	port_device_name = NULL;
	...
	for (auto const &idii : zedevice->IDII) {
		...
		trio_snprintf(sname_pdpfx, sizeof(sname_pdpfx), "%s.input.%s.%s", gi->shortname, gi->PortInfo[port].ShortName,
			      zedevice->ShortName);
		...
		if (idii.Type == IDIT_BUTTON || idii.Type == IDIT_BUTTON_CAN_RAPID || idii.Type == IDIT_BUTTON_ANALOG ||
		    idii.Type == IDIT_AXIS || idii.Type == IDIT_POINTER_X || idii.Type == IDIT_POINTER_Y ||
		    idii.Type == IDIT_AXIS_REL || idii.Type == IDIT_SWITCH) {
			for (unsigned part = 0;
			     part <
			     ((idii.Type == IDIT_BUTTON_CAN_RAPID || idii.Type == IDIT_AXIS || idii.Type == IDIT_AXIS_REL) ? 2 : 1);
			     part++) {
				ButtonInfoCache bic;
				...
				if (idii.Type == IDIT_AXIS_REL) {
					bic.SettingName =
						build_string(sname_pdpfx, ".", idii.SettingName, idii.SettingName[0] ? "_" : "",
							     idii.AxisRel.sname_dir[part]);
					bic.CPName = build_string(idii.Name, " ", idii.AxisRel.name_dir[part]);
				} else if (idii.Type == IDIT_AXIS) {
					bic.SettingName = build_string(sname_pdpfx, ".", idii.SettingName,
								       idii.SettingName[0] ? "_" : "", idii.Axis.sname_dir[part]);
					bic.CPName = build_string(idii.Name, " ", idii.Axis.name_dir[part]);
				} else {
					bic.SettingName =
						build_string(sname_pdpfx, ".", bic.IMType == IMTYPE_BUTTON_RAPID ? "rapid_" : "",
							     idii.SettingName);
					bic.CPName =
						trio_aprintf(_("%s%s%s"), (bic.IMType == IMTYPE_BUTTON_RAPID ? _("Rapid ") : ""),
							     idii.Name, (bic.IMType == IMTYPE_SWITCH) ? _(" Select") : "");
				}
				CleanSettingName(bic.SettingName);
				if (!StringToBCG(&bic.BC, MDFN_GetSettingS(bic.SettingName).c_str(),
						 MDFNI_GetSettingDefault(bic.SettingName).c_str(),
				...
				if (idii.ConfigOrder >= 0 && idii.Type != IDIT_AXIS_REL) {
					...
					pot.first = ((uint64)idii.ConfigOrder << 32) +
						    (part ^ (idii.Type == IDIT_AXIS && (idii.Flags & IDIT_AXIS_FLAG_INVERT_CO)) ^
						     (idii.Type == IDIT_AXIS_REL && (idii.Flags & IDIT_AXIS_REL_FLAG_INVERT_CO)));
	...
	//
	std::sort(bcc_stmp.begin(), bcc_stmp.end(), [](const std::pair<uint64, size_t> &a, const std::pair<uint64, size_t> &b) {
...
static void IncSelectedDevice(unsigned int port)
{
	if (MDFNDnetplay) {
		...
		MDFN_Notify(MDFN_NOTICE_STATUS, _("%s selected on port %d"),
			    CurGame->PortInfo[port].DeviceInfo[PIDC[port].CurrentDeviceIndex].FullName, port + 1);
	}
}
...
#define CKEYDEF(n, d, f, c)                                                                                                        \
	{                                                                                                                          \
		n, gettext_noop(d), "command." n, c, (bool)((f)&CKEYDEF_BYPASSKEYZEROING), (bool)!((f)&CKEYDEF_DANGEROUS),         \
			(bool)((f)&CKEYDEF_TEXTINPUTEXIT)                                                                          \
	}
...
{
	CKeysActive[which] =
		DTestButtonCombo(CKeysConfig[which],
				 (CKeys[which].BypassKeyZeroing && (!EmuKeyboardKeysGrabbed || which == CK_TOGGLE_GRAB)),
				 CKeys[which].TextInputExit);
...
static void CK_UpdateState(bool skipckd_tc)
{
	for (CommandKey i = _CK_FIRST; i < _CK_COUNT; i = (CommandKey)((unsigned)i + 1)) {
		...
		const bool cur_state =
			DTestButtonCombo(CKeysConfig[i],
					 (CKeys[i].BypassKeyZeroing && (!EmuKeyboardKeysGrabbed || i == CK_TOGGLE_GRAB)),
					 CKeys[i].TextInputExit);
...
static void DoKeyStateZeroing(void)
{
	...
	if (IConfig == none) {
		if (Debugger_IsActive()) {
			/*
			static const unsigned sc_um[] = { SCANCODE_F1,	    SCANCODE_F2,      SDL_SCANCODE_F3,	SDL_SCANCODE_F4,
...
static void CheckCommandKeys(void)
{
	if (IConfig >= Port1 && IConfig <= Port13) {
		...
		if (CK_Check((CommandKey)(CK_INPUT_CONFIG1 + i))) {
			CK_PostRemapUpdate(
				(CommandKey)(CK_INPUT_CONFIG1 + i)); // Kind of abusing that function if going by its name, but meh.
	...
	if (IConfig == Command || IConfig == CommandAM) {
		if (ICLatch != -1) {
			...
			MDFN_Notify(MDFN_NOTICE_STATUS, _("Press command key to remap now%s..."),
				    (IConfig == CommandAM) ? _("(AND Mode)") : "");
	...
	if (!Debugger_IsActive()) // We don't want to start button configuration when the debugger is active!
	{
		for (unsigned i = 0; i < 13; i++) {
			if (CK_Check((CommandKey)(CK_INPUT_CONFIG1 + i))) {
				if (i >= CurGame->PortInfo.size())
					...
					MDFN_Notify(MDFN_NOTICE_STATUS, _("No buttons to configure on port %u device \"%s\"!"),
						    i + 1, PIDC[i].Device->FullName);
				} else {
					//SetJoyReadMode(0);
					ConfigDeviceBegin(i);
		...
		if (CK_Check(CK_INPUT_CONFIG_ABD)) {
			MDFN_Notify(MDFN_NOTICE_STATUS, "%u joystick/gamepad analog button(s) detected.",
				    JoystickManager::DetectAnalogButtonsForChangeCheck());
		}
	}
	...
	if (CK_Check(CK_STATE_REWIND_TOGGLE)) {
		...
		MDFN_Notify(MDFN_NOTICE_STATUS, RewindState ? _("State rewinding functionality enabled.") :
							      _("State rewinding functionality disabled."));
	}
	...
	if (CurGame->GameType == GMT_ARCADE && !strcmp(CurGame->shortname, "nes")) {
		...
		if (!strcmp(CurGame->shortname, "nes") && (!strcmp(PIDC[4].Device->ShortName, "bworld") ||
							   (CurGame->cspecial && !MDFN_strazicmp(CurGame->cspecial, "datach")))) {
	...
	for (unsigned int x = 0; x < CurGame->PortInfo.size(); x++) {
		...
		for (size_t bic_i = 0; bic_i < PIDC[x].BIC.size(); bic_i++) {
			...
			case IMTYPE_BUTTON_RAPID:
				*btptr |= (DTestButton(bic.BC, bypass_key_masking) & (rapid >= ((autofirefreq + 1) >> 1)))
					  << (bo & 7);
				break;
			...
			case IMTYPE_AXIS_POS:
				...
				MDFN_en16lsb(btptr,
					     DTestAxis(PIDC[x].BIC[bic_i - 1].BC, bic.BC, bypass_key_masking, PIDC[x].AxisScale));
				break;
			...
			case IMTYPE_AXIS_REL_POS:
				...
				bic.AxisRel.AccumError +=
					(int64)DTestAxisRel(PIDC[x].BIC[bic_i - 1].BC, bic.BC, bypass_key_masking) *
					(int64)floor(0.5 + CurGame->mouse_sensitivity * (1 << 20));
				{
					int32 tosend = bic.AxisRel.AccumError /
						       ((int64)1 << 32); // Division, not simple right arithmetic shift.
					//printf("%lld, %d\n", tosend, bic.AxisRel.AccumError >> 32);
					MDFN_en16lsb(btptr,
						     std::max<int32>(
							     -32768,
							     std::min<int32>(32767,
									     tosend))); // don't assign result of min/max to tosend
			...
			case IMTYPE_SWITCH: {
				...
				if (MDFN_UNLIKELY(
					    cv >=
					    bic.Switch.NumPos)) // Can also be triggered intentionally by a bad save state/netplay.
					fprintf(stderr, "[BUG] cv(%u) >= bic.Switch.NumPos(%u)\n", cv, bic.Switch.NumPos);
				else if (MDFN_UNLIKELY(cv != bic.Switch.LastPos)) {
					MDFN_Notify(MDFN_NOTICE_STATUS, _("%s %u: %s: %s selected."), PIDC[x].Device->FullName,
						    x + 1, bic.IDII->Name, bic.IDII->Switch.Pos[cv].Name);
		...
		for (auto &sic : PIDC[x].SIC) {
			...
			if (MDFN_UNLIKELY(cv >= sic.StatusNumStates))
				...
				MDFN_Notify(MDFN_NOTICE_STATUS, _("%s %u: %s: %s"), PIDC[x].Device->FullName, x + 1, sic.IDII->Name,
					    sic.IDII->Status.States[cv].Name);
...
static bool subcon(const char *text, std::vector<ButtConfig> &bcg, const bool commandkey, const bool AND_Mode)
{
	while (1) {
		...
		if (!KBMan::Do_BC_ChangeCheck(&subcon_bc) && !MouseMan::Do_BC_ChangeCheck(&subcon_bc) &&
		    !JoystickManager::Do_BC_ChangeCheck(&subcon_bc))
			return false;
		//
		//
		if (subcon_bcg.size()) {
			...
			if (c.DeviceType == p.DeviceType && c.DeviceNum == p.DeviceNum && c.ButtonNum == p.ButtonNum &&
...
static INLINE void MakeSettingsForDevice(const MDFNGI *system, const int w, const InputDeviceInfoStruct *info,
					 const DefaultSettingsMeow *defs)
{
	size_t def_butti = 0;
	bool analog_scale_made = false;
	for (size_t x = 0; x < info->IDII.size(); x++) {
		if (info->IDII[x].Type != IDIT_BUTTON && info->IDII[x].Type != IDIT_BUTTON_CAN_RAPID &&
		    info->IDII[x].Type != IDIT_BUTTON_ANALOG && info->IDII[x].Type != IDIT_AXIS &&
		    info->IDII[x].Type != IDIT_POINTER_X && info->IDII[x].Type != IDIT_POINTER_Y &&
		    info->IDII[x].Type != IDIT_AXIS_REL && info->IDII[x].Type != IDIT_SWITCH)
			continue;
		...
		if (info->IDII[x].Type == IDIT_AXIS) {
			for (unsigned part = 0; part < 2; part++) {
				...
				tmp_setting.name = CleanSettingName(
					build_string(system->shortname, ".input.", system->PortInfo[w].ShortName, ".",
						     info->ShortName, ".", info->IDII[x].SettingName,
						     info->IDII[x].SettingName[0] ? "_" : "", info->IDII[x].Axis.sname_dir[part]));
				tmp_setting.description =
					build_string(system->shortname, ", ", system->PortInfo[w].FullName, ", ", info->FullName,
						     ": ", info->IDII[x].Name, info->IDII[x].Name[0] ? " " : "",
						     info->IDII[x].Axis.name_dir[part]);
				tmp_setting.type = MDFNST_STRING;
				tmp_setting.default_value = "";

				tmp_setting.flags =
					MDFNSF_SUPPRESS_DOC | MDFNSF_CAT_INPUT_MAPPING | MDFNSF_FREE_NAME | MDFNSF_FREE_DESC;
			...
			if ((info->IDII[x].Flags & IDIT_AXIS_FLAG_SQLR) && !analog_scale_made) {
				...
				tmp_setting.name =
					CleanSettingName(build_string(system->shortname, ".input.", system->PortInfo[w].ShortName,
								      ".", info->ShortName, ".axis_scale"));
				tmp_setting.description = trio_aprintf(gettext_noop("Analog axis scale coefficient for %s on %s."),
								       info->FullName, system->PortInfo[w].FullName);
				tmp_setting.description_extra = NULL;
			...
			for (unsigned part = 0; part < 2; part++) {
				...
				tmp_setting.name =
					CleanSettingName(build_string(system->shortname, ".input.", system->PortInfo[w].ShortName,
								      ".", info->ShortName, ".", info->IDII[x].SettingName,
								      info->IDII[x].SettingName[0] ? "_" : "",
								      info->IDII[x].AxisRel.sname_dir[part]));
				tmp_setting.description =
					build_string(system->shortname, ", ", system->PortInfo[w].FullName, ", ", info->FullName,
						     ": ", info->IDII[x].Name, info->IDII[x].Name[0] ? " " : "",
						     info->IDII[x].AxisRel.name_dir[part]);
			...
			tmp_setting.name =
				CleanSettingName(build_string(system->shortname, ".input.", system->PortInfo[w].ShortName, ".",
							      info->ShortName, ".", info->IDII[x].SettingName));
			tmp_setting.description = build_string(system->shortname, ", ", system->PortInfo[w].FullName, ", ",
							       info->FullName, ": ", info->IDII[x].Name);
		...
		if (info->IDII[x].Type == IDIT_BUTTON_CAN_RAPID) {
			...
			tmp_setting.name =
				CleanSettingName(build_string(system->shortname, ".input.", system->PortInfo[w].ShortName, ".",
							      info->ShortName, ".rapid_", info->IDII[x].SettingName));
			tmp_setting.description = build_string(system->shortname, ", ", system->PortInfo[w].FullName, ", ",
							       info->FullName, ": Rapid ", info->IDII[x].Name);
			tmp_setting.type = MDFNST_STRING;
			...
			tmp_setting.name =
				CleanSettingName(build_string(system->shortname, ".input.", system->PortInfo[w].ShortName, ".",
							      info->ShortName, ".", info->IDII[x].SettingName, ".defpos"));
			tmp_setting.description =
				trio_aprintf(gettext_noop("Default position for switch \"%s\"."), info->IDII[x].Name);
			tmp_setting.description_extra = gettext_noop(
			...
			tmp_setting.flags = ((info->IDII[x].Flags & IDIT_FLAG_AUX_SETTINGS_UNDOC) ? MDFNSF_SUPPRESS_DOC : 0) |
					    MDFNSF_FREE_NAME | MDFNSF_FREE_DESC | MDFNSF_FREE_ENUMLIST;
			{
				MDFNSetting_EnumList *el = (MDFNSetting_EnumList *)calloc(info->IDII[x].Switch.NumPos + 1,
											  sizeof(MDFNSetting_EnumList));
...
static INLINE void MakeSettingsForPort(const MDFNGI *system, const int w, const InputPortInfoStruct *info, const T &defset)
{
	if (info->DeviceInfo.size() > 1) {
		...
		for (unsigned device = 0; device < info->DeviceInfo.size(); device++) {
			...
			if (info->DeviceInfo[device].Description ||
			    (info->DeviceInfo[device].Flags & InputDeviceInfoStruct::FLAG_KEYBOARD)) {
				EnumList[device].description_extra = build_string(
					info->DeviceInfo[device].Description ? info->DeviceInfo[device].Description : "",
					(info->DeviceInfo[device].Description &&
					 (info->DeviceInfo[device].Flags & InputDeviceInfoStruct::FLAG_KEYBOARD)) ?
						"\n" :
						"",
					(info->DeviceInfo[device].Flags & InputDeviceInfoStruct::FLAG_KEYBOARD) ?
						_("Emulated keyboard key state is not updated unless input grabbing(by default, mapped to CTRL+SHIFT+Menu) is toggled on; refer to the main documentation for details.") :
						"");
			}
		}
		...
		tmp_setting.flags = MDFNSF_EMU_STATE | MDFNSF_UNTRUSTED_SAFE | MDFNSF_FREE_NAME | MDFNSF_FREE_DESC |
				    MDFNSF_FREE_ENUMLIST | MDFNSF_FREE_ENUMLIST_STRING | MDFNSF_FREE_ENUMLIST_DESC |
				    MDFNSF_FREE_ENUMLIST_DESC_EXTRA;
		tmp_setting.description_extra = NULL;
	...
	for (unsigned device = 0; device < info->DeviceInfo.size(); device++) {
		...
		trio_snprintf(setting_def_search, sizeof(setting_def_search), "%s.input.%s.%s", system->shortname,
			      system->PortInfo[w].ShortName, info->DeviceInfo[device].ShortName);
...
static void CalcWMInputBehavior_Sub(const InputMappingType IMType, const ButtConfig &bc, bool *CursorNeeded, bool *MouseAbsNeeded,
				    bool *MouseRelNeeded)
{
	const bool is_mouse_mapping = (bc.DeviceType == BUTTC_MOUSE);
	const bool is_mouse_cursor_mapping =
		is_mouse_mapping && ((bc.ButtonNum & MouseMan::MOUSE_BN_TYPE_MASK) == MouseMan::MOUSE_BN_TYPE_CURSOR);
	const bool is_mouse_rel_mapping =
		is_mouse_mapping && ((bc.ButtonNum & MouseMan::MOUSE_BN_TYPE_MASK) == MouseMan::MOUSE_BN_TYPE_REL);
