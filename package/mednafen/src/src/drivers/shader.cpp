...
OpenGL_Blitter_Shader::OpenGL_Blitter_Shader(OpenGL_Blitter *in_oblt, ShaderType shader_type, const ShaderParams &in_params)
	: oblt(in_oblt), params(in_params)
{
	OurType = shader_type;
...
void OpenGL_Blitter_Shader::ShaderBegin(const int gl_screen_w, const int gl_screen_h, const MDFN_Rect *rect,
					const MDFN_Rect *dest_rect, int tw, int th, int orig_tw, int orig_th, unsigned rotated)
{
	oblt->p_glEnable(GL_FRAGMENT_PROGRAM_ARB);
	...
	if (OurType == SHADER_SCALE2X) {
		...
		oblt->p_glUniform1fARB(oblt->p_glGetUniformLocationARB(CSP[0].p, "XSharp"),
				       std::max<float>(1.0, ipc_drw / rect->w * 0.25));
		oblt->p_glUniform1fARB(oblt->p_glGetUniformLocationARB(CSP[0].p, "YSharp"),
				       std::max<float>(1.0, ipc_drh / rect->h * 0.25));
