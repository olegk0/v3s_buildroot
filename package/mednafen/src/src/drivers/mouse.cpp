...
void UpdateMice(void)
{
	...
	Video_PtoV(MouseState.x, MouseState.y, &MouseDataPointer[0], &MouseDataPointer[1]);
	{
		...
		MouseDataPointerAsAxis[0] = std::min<int32>(
			32767, std::max<int32>(-32768, 32768 * (MouseDataPointer[0] * 2 - 1) * CurGame->nominal_width / nw_nh_min));
		MouseDataPointerAsAxis[1] =
			std::min<int32>(32767, std::max<int32>(-32768, 32768 * (MouseDataPointer[1] * 2 - 1) *
									       CurGame->nominal_height / nw_nh_min));
...
void Event(const SDL_Event *event)
{
	...
	case SDL_BUTTON_WHEELUP:
		...
		MouseState.rel_accum[3] += 1; //event->wheel.y;
		break;
...
std::string BNToString(const uint32 bn)
{
	...
	case MOUSE_BN_TYPE_REL:
		trio_snprintf(tmp, sizeof(tmp), "%s_%s%s", (type == MOUSE_BN_TYPE_REL) ? "rel" : "cursor", (index & 1) ? "y" : "x",
