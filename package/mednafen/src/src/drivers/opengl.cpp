...
void OpenGL_Blitter::ReadPixels(MDFN_Surface *surface, const MDFN_Rect *rect)
{
	...
	for (int32 y = 0; y < surface->h / 2; y++) {
		...
		for (int32 x = 0; x < surface->w;) {
			...
			memcpy(&surface->pixels[y * surface->pitchinpix + x],
			       &surface->pixels[(surface->h - 1 - y) * surface->pitchinpix + x], tw * sizeof(uint32));
...
void OpenGL_Blitter::BlitOSD(const MDFN_Surface *surface, const MDFN_Rect *rect, const MDFN_Rect *dest_rect,
			     const bool source_alpha)
{
	...
	if (tmpwidth > MaxTextureSize || tmpheight > MaxTextureSize) {
		...
		p_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, rect->w, rect->h, OSDPixelFormat, OSDPixelType,
				  surface->pixels + rect->x + rect->y * surface->pitchinpix);
...
void OpenGL_Blitter::DrawLinearIP(const unsigned UsingIP, const unsigned rotated, const MDFN_Rect *tex_src_rect,
				  const MDFN_Rect *dest_rect, const uint32 tmpwidth, const uint32 tmpheight)
{
	...
	reversi = (rotated == MDFN_ROTATE270 && UsingIP == VIDEOIP_LINEAR_X) ||
		  (rotated == MDFN_ROTATE90 && UsingIP == VIDEOIP_LINEAR_Y);
...
void OpenGL_Blitter::Blit(const MDFN_Surface *src_surface, const MDFN_Rect *src_rect, const MDFN_Rect *dest_rect,
			  const MDFN_Rect *original_src_rect, int InterlaceField, int UsingIP, int rotated)
{
	...
	if (tex_src_rect.w == 0 || tex_src_rect.h == 0 || dest_rect->w == 0 || dest_rect->h == 0 || original_src_rect->w == 0 ||
	    original_src_rect->h == 0) {
		printf("[BUG] OpenGL blitting nothing? --- %d:%d %d:%d %d:%d\n", tex_src_rect.w, tex_src_rect.h, dest_rect->w,
		       dest_rect->h, original_src_rect->w, original_src_rect->h);
		return;
	}

	if (src_surface->pixels16)
		src_pixies = src_surface->pixels16 + tex_src_rect.x +
			     (tex_src_rect.y + (InterlaceField & ShaderIlace)) * src_surface->pitchinpix;
	else
		src_pixies = src_surface->pixels + tex_src_rect.x +
			     (tex_src_rect.y + (InterlaceField & ShaderIlace)) * src_surface->pitchinpix;
	tex_src_rect.x = 0;
	tex_src_rect.y = 0;
		...
		if (ImageSizeChange && (shader || UsingIP)) {
			...
			if (DummyBlack) // If memory allocation failed for some reason, don't clean the texture. :(
			{
				if (tex_src_rect.w < tmpwidth) {
					...
					p_glTexSubImage2D(GL_TEXTURE_2D, 0, tex_src_rect.w, 0, 1, tex_src_rect.h, GL_RGBA,
							  GL_UNSIGNED_BYTE, DummyBlack);
				}
				if (tex_src_rect.h < tmpheight) {
					//puts("Y");
					p_glPixelStorei(GL_UNPACK_ROW_LENGTH, tex_src_rect.w);
					p_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, tex_src_rect.h, tex_src_rect.w, 1, GL_RGBA,
							  GL_UNSIGNED_BYTE, DummyBlack);
				}
			} // end if(DummyBlack)
		}
	...
	if (shader)
		shader->ShaderBegin(gl_screen_w, gl_screen_h, src_rect, dest_rect, tmpwidth, tmpheight,
				    round((double)tmpwidth * original_src_rect->w / tex_src_rect.w),
				    round((double)tmpheight * (original_src_rect->h >> ShaderIlace) / tex_src_rect.h), rotated);

	p_glPixelStorei(GL_UNPACK_ROW_LENGTH, src_surface->pitchinpix << ShaderIlace);

	p_glTexSubImage2D(GL_TEXTURE_2D, 0, tex_src_rect.x, tex_src_rect.y, tex_src_rect.w, tex_src_rect.h, PixelFormat, PixelType,
			  src_pixies);
...
static bool CheckAlternateFormat(const uint32 version_h)
{
	...
	{
#if defined(__amd64__) || defined(__x86_64__) || defined(_M_AMD64) || defined(__386__) || defined(__i386__) || defined(__i386) ||  \
...
OpenGL_Blitter::OpenGL_Blitter(int scanlines, ShaderType pixshader, const ShaderParams &shader_params, MDFN_PixelFormat *game_pf,
			       MDFN_PixelFormat *osd_pf, uint32 preferred_format)
{
	try {
...
#define LFG(x)                                                                                                                     \
	if (!(p_##x = (x##_Func)SDL_GL_GetProcAddress(#x))) {                                                                      \
		throw MDFN_Error(0, _("Error getting proc address for: %s\n"), #x);                                                \
	}
#define LFGN(x) p_##x = (x##_Func)SDL_GL_GetProcAddress(#x)
		...
		if (pixshader != SHADER_NONE) {
			...
			SupportNPOT =
				0; // Our pixel shaders don't work right with NPOT textures(for them to do so would probably necessitate rewriting them to use texelFetch)
		...
		while (MaxTextureSize) {
			...
			p_glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_RGBA, MaxTextureSize, MaxTextureSize, 0, GL_RGBA,
		...
		if (MaxTextureSize < 256) {
			MDFN_printf(_(
				"Warning:  Maximum texture size is reported as being less than 256, but we can't handle that.\n"));
			MaxTextureSize = 256;
		}
