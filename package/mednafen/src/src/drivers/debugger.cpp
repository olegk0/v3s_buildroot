	...
	for (last_x = x = 0; x < len; x++) {
		if (zestring[x] == '-')
			...
			if (two_parter) {
				...
				if (!logical) {
					if (trio_sscanf(zestring + last_x, "%63[^-]%*[-]%63s", sa1, sa2) < 2)
						continue;

					//printf("%s %s\n", sa1, sa2);
					A1 = ParsePhysAddr(sa1);
					A2 = ParsePhysAddr(sa2);
				} else if (trio_sscanf(zestring + last_x, "%x%*[-]%x", &A1, &A2) < 2)
					continue;
				...
				if (!logical) {
					...
					A1 = ParsePhysAddr(sa1);
				} else if (trio_sscanf(zestring + last_x, "%x", &A1) != 1)
					continue;
...
static void DrawZP(MDFN_Surface *surface, const int32 base_x, const int32 base_y)
{
	...
	for (int y = -1; y < 17; y++)
		for (int x = -1; x < 17; x++) {
			...
			DrawText(surface, base_x + (x + 1) * 13, base_y + (y + 1) * 10, tbuf, pf_cache.MakeColor(r, g, b, 0xFF),
				 MDFN_FONT_5x7);
			if (NeedInc)
				addr++;
		}
...
static SDLKey PromptTAKC = SDLK_UNKNOWN;
			...
			if (pstring != TraceLogSpec || !TraceLog) {
				...
				if (num >= 1) {
					try {
						...
						TraceLog->print_format(
							"[ADDRESS]: [INSTRUCTION]   [REGISTERS(before instruction exec)]");
...
void Debugger_GT_Draw(void)
{
	...
	while (DisBytes > 0) {
		...
		// Handling resynch address ->0 wrapping is a bit more complex...
		{
			...
			// Handle comment forced resynchronizations
			{
				...
				for (it = Comments.begin(); it != Comments.end(); it++) {
					uint32 comment_distance =
						((uint32)it->first - A - 1) & ((1ULL << CurGame->Debugger->LogAddrBits) - 1);
		...
		if (NewEntry.A == DisAddr && (NewEntry.COffs == DisCOffs ||
					      indexcow == -1)) // Also handles case where comments disappear out from underneath us.
			indexcow = DisBuffer.size();
	...
	for (int x = 0; x < DIS_ENTRIES; x++) {
		...
		} else // Disassembly
		{
			...
			trio_snprintf(addr_text, sizeof(addr_text), " %0*X%s", (CurGame->Debugger->LogAddrBits + 3) / 4, dis_A,
				      (DisBuffer[dbi].ForcedResync ? "!!" : ": "));
	...
	for (unsigned int rp = 0; rp < CurGame->Debugger->RegGroups->size(); rp++)
		Regs_DrawGroup((*CurGame->Debugger->RegGroups)[rp], surface, rect->w - RegsTotalWidth + RegsColsPixOffset[rp],
			       (InRegs && RegsPosX == rp) ? (int)RegsPosY : -1, RegsWhichFont[rp]); // 175
	...
	if (InRegs) {
		DrawText(surface, 0, (rect->h - (moo + 2) * 7), CurRegLongName, pf_cache.MakeColor(0xa0, 0xa0, 0xFF, 0xFF),
			 MDFN_FONT_5x7, surface->w);
		DrawText(surface, 0, (rect->h - (moo + 1) * 7), CurRegDetails, pf_cache.MakeColor(0x60, 0xb0, 0xFF, 0xFF),
			 MDFN_FONT_5x7, surface->w);
		...
		const uint32 hcolors[2] = { pf_cache.MakeColor(0x60, 0xb0, 0xfF, 0xFF),
					    pf_cache.MakeColor(0xb0, 0x70, 0xfF, 0xFF) };
		...
		for (int i = (int)btrace.size() - 1; i >= 0; i--) {
			...
			strbuf_len =
				(GetTextPixLength(strbuf[0], MDFN_FONT_5x7) + GetTextPixLength(strbuf[1], MDFN_FONT_5x7) +
				 GetTextPixLength(strbuf[2], MDFN_FONT_5x7) + 5 + GetTextPixLength(strbuf[3], MDFN_FONT_5x7) + 4) /
				5;
			...
			x_tmp = bt_x + col * 5;
			y_tmp = bt_y + row * 10;
			x_tmp += DrawText(surface, x_tmp, y_tmp, strbuf[0],
					  (btrace[i].count > 1) ? pf_cache.MakeColor(0xe0, 0xe0, 0x00, 0xFF) : hcolors[color_osc],
					  MDFN_FONT_5x7);
			x_tmp += DrawText(surface, x_tmp, y_tmp, strbuf[1],
					  btrace[i].code[0] ? pf_cache.MakeColor(0xb0, 0xFF, 0xff, 0xFF) :
							      pf_cache.MakeColor(0xb0, 0xb0, 0xff, 0xFF),
					  MDFN_FONT_5x7);

			color_osc = !color_osc;

			x_tmp += DrawText(surface, x_tmp, y_tmp, strbuf[2],
					  (btrace[i].count > 1) ? pf_cache.MakeColor(0xe0, 0xe0, 0x00, 0xFF) : hcolors[color_osc],
					  MDFN_FONT_5x7);
			x_tmp += 2;
			x_tmp += DrawText(surface, x_tmp, y_tmp, strbuf[3], pf_cache.MakeColor(0x60, 0x70, 0x80, 0xFF),
					  MDFN_FONT_5x7);
			x_tmp += 3;
	...
	//
	{
		...
		for (int y = 0; y < mw_rows; y++) {
			...
			trio_snprintf(tbuf, sizeof(tbuf), "%0*X: ", std::max<int>(4, (ewa_bits + 3) / 4),
				      (ewa + y * bytes_per_row) & ewa_mask);

			row_x += DrawText(surface, row_x, row_y, tbuf, pf_cache.MakeColor(0xa0, 0xa0, 0xFF, 0xFF), fontid);
			for (int x = 0; x < bytes_per_row; x++) {
				uint8 zebyte =
					CurGame->Debugger->MemPeek((ewa + y * bytes_per_row + x) & ewa_mask, 1, 1, WatchLogical);
...
void Debugger_GT_ModOpacity(int deltalove)
{
	...
	if (DebuggerOpacity < 0)
		DebuggerOpacity = 0;
	if (DebuggerOpacity > 0xFF)
		DebuggerOpacity = 0xFF;
}
	...
	if (event->type == SDL_KEYDOWN) {
		if ((event->key.keysym.mod & KMOD_LALT) && !(event->key.keysym.mod & KMOD_CTRL)) {
			...
			case SDLK_2:
				...
				break;
			case SDLK_3:
				if (CurGame->Debugger->AddressSpaces->size()) {
					WhichMode = 2;
					GfxDebugger_SetActive(FALSE);
			...
			case SDLK_PAGEUP:
				if (event->key.keysym.mod & KMOD_SHIFT) {
					...
					if (WatchLogical)
						...
						WatchAddrPhys = (WatchAddrPhys - change) &
								(((uint64)1 << CurGame->Debugger->PhysAddrBits) - 1);
				} else
			...
			case SDLK_UP:
				if (InRegs) {
					...
					if (event->key.keysym.mod & KMOD_SHIFT) {
						if (WatchLogical)
							WatchAddr =
								(WatchAddr - 0x10) & ((1ULL << CurGame->Debugger->LogAddrBits) - 1);
						else
							WatchAddrPhys = (WatchAddrPhys - 0x10) &
									(((uint64)1 << CurGame->Debugger->PhysAddrBits) - 1);
					} else {
						NeedDisAddrChange = -1;
					}
			...
			case SDLK_RETURN: {
				...
				if (event->key.keysym.mod & KMOD_SHIFT) {
					...
					if (InRegs && event->key.keysym.sym != SDLK_g) {
						...
						ptext = CurRegIP->name;
						const int rval_fw = (CurRegIP->bsize & 0x100) ?
									    (((CurRegIP->bsize & 0xFF) + 3) / 4) :
									    (CurRegIP->bsize * 2);
