...
void MemDebugger::ICV_Init(const char *newcode)
{
	...
	if ((new_ict_game_to_utf8 = iconv_open("UTF-8", newcode)) == (iconv_t)-1 ||
	    (new_ict_utf8_to_game = iconv_open(newcode, "UTF-8")) == (iconv_t)-1) {
...
class MemDebuggerPrompt : public HappyPrompt {
    public:
	MemDebuggerPrompt(MemDebugger *memdbg_in, const std::string &ptext, const std::string &zestring)
		: HappyPrompt(ptext, zestring), memdbg(memdbg_in)
	{
	}
	~MemDebuggerPrompt()
	{
		...
		if (which == Goto || which == GotoDD) {
			...
			if (!acceptable)
				...
				while (a <= A2) {
					...
					if (read_len > 0)
						ASpace->PutAddressSpaceBytes(ASpace->name.c_str(), a, read_len, 1, true,
									     read_buffer);

					a += read_len;

					if (read_len != to_read) {
						error_string = trio_aprintf(_("Warning: unexpected EOF(short by %08llx byte(s))"),
...
INLINE int32 MemDebugger::DrawWaveform(MDFN_Surface *surface, const int32 base_y, const uint32 hcenterw)
{
	...
	MDFN_DrawFillRect(surface, xo + 1, yo + 1, area_w - 2, area_h - 2, surface->MakeColor(0x30, 0x30, 0x30, 0xFF),
			  surface->MakeColor(0, 0, 0, 0xFF));
	xo += 2;
	yo += 2;
...
INLINE void MemDebugger::DrawAtCursorInfo(MDFN_Surface *surface, const int32 base_y, const uint32 hcenterw)
{
	...
	if (GoGoPowerDD[CurASpace]) {
		...
		trio_snprintf(ggddstr, sizeof(ggddstr), "(0x%0*X+0x%0*X)", curpos_fw, (unsigned)GoGoPowerDD[CurASpace], curpos_fw,
			      (unsigned)(curpos - GoGoPowerDD[CurASpace]));
	...
	cpplen = DrawText(surface, x, y, cpstr, surface->MakeColor(0xA0, 0xA0, 0xFF, 0xFF), fontid);
	{
		...
		if (ict_game_to_utf8 != (iconv_t)-1) {
			...
			DrawText(surface, x + cpplen, y - 2, textbuf, surface->MakeColor(0xEF, 0xEF, 0xEF, 0xFF),
...
void MemDebugger::Draw(MDFN_Surface *surface, const MDFN_Rect *rect, const MDFN_Rect *screen_rect)
{
	...
	for (int y = 0; y < numrows; y++) {
		...
		for (int x = 0; x < byte_bpr; x++) {
			...
			if (Ameow == test_match_pos) {
				if (InEditMode) {
					if (curtime & 0x80) {
						...
						if (InTextArea) {
							pix_offset += byte_bpr * byte_hex_spacing + byte_hex_right_padding +
								      x * byte_char_spacing +
								      ((byte_bpr - 1) / 4) * byte_hex_group_pad;
							DrawText(surface, pix_offset, text_y + byte_char_y_adjust, "▉",
								 pf_cache.MakeColor(0xFF, 0xFF, 0xFF, 0xFF), byte_char_font);
						} else {
							pix_offset += (LowNib ? byte_hex_font_width : 0) + x * byte_hex_spacing +
								      (x / 4) * byte_hex_group_pad;
							DrawText(surface, pix_offset, text_y, "▉",
								 pf_cache.MakeColor(0xFF, 0xFF, 0xFF, 0xFF), byte_hex_font);
						}
					}
				}
			...
			// hex display
			DrawText(surface, alen + x * byte_hex_spacing + ((x / 4) * byte_hex_group_pad), text_y, quickbuf, bcolor,
				 byte_hex_font);

			// ASCII display
			DrawText(surface,
				 alen + byte_bpr * byte_hex_spacing + byte_hex_right_padding + x * byte_char_spacing +
					 ((byte_bpr - 1) / 4) * byte_hex_group_pad,
				 text_y + byte_char_y_adjust, ascii_str, acolor, byte_char_font);
			Ameow++;
		}
	...
	if (error_string) {
		...
		if ((int32)(curtime - error_time) >= 4000) {
			...
			MDFN_DrawFillRect(surface, 0, (rect->h - 13 - 2), rect->w, 13 + 2,
					  pf_cache.MakeColor(0xFF, 0xFF, 0xFF, 0xFF), pf_cache.MakeColor(0, 0, 0, 0xFF));
			DrawText(surface, 0, (rect->h - 13 - 1), error_string, pf_cache.MakeColor(0xFF, 0x00, 0x00, 0xFF),
				 MDFN_FONT_6x13_12x13, rect->w);
		}
	}
}
...
*/
	...
	case SDL_KEYDOWN:
		...
		if (InPrompt) {
		...
		} else if (InEditMode && !InTextArea && ((ks >= SDLK_0 && ks <= SDLK_9) || (ks >= SDLK_a && ks <= SDLK_f))) {
			uint8 tc = 0;
			uint8 meowbyte = 0;
			...
			if (!LowNib)
				ChangePos(1);
		} else if (InEditMode && InTextArea && ks != SDLK_TAB && ks != SDLK_RETURN && ks != SDLK_INSERT && ks != SDLK_END &&
			   ks != SDLK_HOME && ks != SDLK_PAGEUP && ks != SDLK_PAGEDOWN && ks != SDLK_UP && ks != SDLK_DOWN &&
			   ks != SDLK_LEFT && ks != SDLK_RIGHT) {
			//
		} else
			...
			case SDLK_END:
				ASpacePos[CurASpace] =
					(SizeCache[CurASpace] - (byte_bpr * byte_maxrows / 2)) % SizeCache[CurASpace];
				LowNib = false;
				break;
...
MemDebugger::MemDebugger()
	: AddressSpaces(NULL), ASpace(NULL), IsActive(false), CurASpace(0), LowNib(false), InEditMode(false), InTextArea(false),
	  error_string(NULL), error_time(-1), ict_game_to_utf8((iconv_t)-1), ict_utf8_to_game((iconv_t)-1), InPrompt(None),
	  myprompt(NULL), PromptTAKC(SDLK_UNKNOWN)
{
	if (CurGame->Debugger) {
		...
		try {
			...
			try {
				ICV_Init(MDFNI_GetSettingDefault(std::string(CurGame->shortname) + "." + "debugger.memcharenc")
						 .c_str());
			} catch (...) {
			}
