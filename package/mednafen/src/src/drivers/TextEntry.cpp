...
void TextEntry::Draw(MDFN_Surface *surface, const MDFN_Rect &rect, const char *prefix_str, const uint32 color,
		     const uint32 shadcolor, const uint32 font)
{
...
bool TextEntry::Event(const SDL_Event *event)
{
	...
	if (event->type == SDL_KEYDOWN) {
		...
		if ((sym == SDLK_v && ctrl && !shift) || (sym == SDLK_INSERT && !ctrl && shift)) {
			/*
			...
			case SDLK_DELETE:
				if (kb_buffer.size() && kb_cursor_pos < kb_buffer.size()) {
					if (ctrl && !shift)
						...
						} while (kb_buffer.size() && kb_cursor_pos < kb_buffer.size() &&
							 UCPIsCombining(kb_buffer[kb_cursor_pos]));
					}
				}
				break;
			}
