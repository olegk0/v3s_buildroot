...
const  int        sdlSoundCapacity = sdlSoundSamples * 64;
...
//    printf("speed:%d  systemFrameSkip:%d\n",speed,systemFrameSkip);
    ...
    if(speed >= 110) {//98
      ...
      else if(systemFrameSkip < 10)//9
        frameskipadjust--;

      if(frameskipadjust <= -2) {
        frameskipadjust += 2;
        if(systemFrameSkip < 10)//9
          systemFrameSkip++;
      }
    }
  }
