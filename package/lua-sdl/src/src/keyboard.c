...
static int
l_enableUnicode(lua_State* L)
{
  int enable = luaL_checkinteger(L, 1);

  return commonPush(L, "i", SDL_EnableUNICODE(enable));
}
...
{
  SDLKey kc = luaL_checkinteger(L, 1);
 ...
 */
static int
l_setKeyRepeat(lua_State* L)
{
  int delay = luaL_checkinteger(L, 1);
  int interval = luaL_checkinteger(L, 2);

  return commonPush(L, "i", SDL_EnableKeyRepeat(delay, interval));
}

static int
l_getKeyRepeat(lua_State* L)
{
  int delay;
  int interval;

  SDL_GetKeyRepeat(&delay, &interval);
  return commonPush(L, "ii", delay, interval);
}
...
{
  SDLMod state = commonGetEnum(L, 1);
...
const CommonEnum KeyboardCodes[] = {
  ...
  // menu{ "Application", SDLK_APPLICATION },
  { "Power", SDLK_POWER },
 ...
 * SDL.keymod
 */
const CommonEnum KeyboardModifiers[] = { { "None", KMOD_NONE },
...
/*
const CommonEnum KeyEvType[] = { { "Pressed", 1 },
                                 { "Repeat", 2 },
                                 { "Released", 3 },
                                 { NULL, -1 } };
                                 */