...
audioCallback(void* userdata, Uint8* stream, int length)
{
  ...
  if (lua_pcall(device->L, 1, 1, 0) != LUA_OK) {
    // TODO SDL_LogCritical(SDL_LOG_CATEGORY_SYSTEM, "%s",
    // lua_tostring(device->L, -1));
    lua_pop(device->L, 1);
...
mixAudio(lua_State* L /*, int use_format*/)
{
  ...
  /*  else
      SDL_MixAudioFormat((Uint8*)data, (const Uint8*)src, format, length,
     volume);
  */
...
l_openAudio(lua_State* L)
{
  ...
  device->desired.callback = audioCallback;
...
l_getAudioDriver(lua_State* L)
{
  ...
  if (SDL_AudioDriverName(name, sizeof(name) - 1) == NULL)
    return commonPushSDLError(L, 1);
 ...
 * SDL.audioStatus
 */
const CommonEnum AudioStatus[] = { { "Stopped", SDL_AUDIO_STOPPED },
...
l_audiodev_queue(lua_State* L)
{
  ...
  if (dev->isdevice) {
    ...
    return commonPush(
      L, "ns", "Must be an AudioDevice (opened with SDL.openAudioDevice).");
  }
}
...
l_audiodev_dequeue(lua_State* L)
{
  ...
  if (dev->isdevice && dev->iscapture) {
    ...
    return commonPush(
      L,
      "ns",
      "Must be a capture AudioDevice (opened with SDL.openAudioDevice).");
  }
}
#endif
...
l_audiodev_eq(lua_State* L)
{
  ...
  return commonPush(L, "b", dev1->callback == dev2->callback); // TODO
}
...
l_audiodev_tostring(lua_State* L)
{
  ...
  for (i = 0; AudioStatus[i].name != NULL; ++i)
    if (AudioStatus[i].value == (int)st) {
      status = AudioStatus[i].name;
      break;
    }

  lua_pushfstring(L, "audio device status: %s", status);
...
static const luaL_Reg AudiodevMetamethods[] = { { "__eq", l_audiodev_eq },
                                                { "__gc", l_audiodev_gc },
                                                { "__tostring",
                                                  l_audiodev_tostring },
                                                { NULL, NULL } };

const CommonObject AudioObject = { "AudioDevice",
                                   AudiodevMethods,
                                   AudiodevMetamethods };
