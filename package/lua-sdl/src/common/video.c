...
videoGetColorHex(lua_State* L, int index)
{
  ...
  return c.unused << 24 | c.r << 16 | c.g << 8 | c.b;
}
...
videoGetColorRGB(lua_State* L, int index)
{
  ...
  if (lua_type(L, index) == LUA_TNUMBER) {
    ...
    c.unused = tableGetInt(L, index, "a");
  }
...
{
  lua_pushinteger(L, (c->unused << 24) | (c->r << 16) | (c->g << 8) | c->b);
}
...
videoPushColorRGB(lua_State* L, const SDL_Color* color)
{
  ...
  tableSetInt(L, -1, "a", color->unused);
}
...
videoGetDisplayMode(lua_State* L, int idx, SDL_VideoInfo* vi)
{
  ...
  // vi->format = tableGetInt(L, idx, "format");
  vi->current_w = tableGetInt(L, idx, "w");
  vi->current_h = tableGetInt(L, idx, "h");
  // vi->refresh_rate = tableGetInt(L, idx, "refreshRate");
  // vi->driverdata = NULL;
}
...
videoPushDisplayMode(lua_State* L, const SDL_VideoInfo* vi)
{
  ...
  tableSetInt(L, -1, "w", vi->current_w);
  tableSetInt(L, -1, "h", vi->current_h);
  // tableSetInt(L, -1, "format", vi->format);
  // tableSetInt(L, -1, "refreshRate", vi->refresh_rate);
}
