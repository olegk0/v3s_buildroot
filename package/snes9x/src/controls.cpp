...
static const char* color_names[32] = {
    "Trans",      "Black",   "25Grey",    "50Grey", "75Grey", "White",
    "Red",        "Orange",  "Yellow",    "Green",  "Cyan",   "Sky",
    "Blue",       "Violet",  "MagicPink", "Purple", NULL,     "tBlack",
    "t25Grey",    "t50Grey", "t75Grey",   "tWhite", "tRed",   "tOrange",
    "tYellow",    "tGreen",  "tCyan",     "tSky",   "tBlue",  "tViolet",
    "tMagicPink", "tPurple"};
...
#define THE_COMMANDS                                                          \
  S(BeginRecordingMovie), S(ClipWindows), S(Debugger), S(DecEmuTurbo),        \
      S(DecFrameRate), S(DecFrameTime), S(DecTurboSpeed), S(EmuTurbo),        \
      S(EndRecordingMovie), S(ExitEmu), S(IncEmuTurbo), S(IncFrameRate),      \
      S(IncFrameTime), S(IncTurboSpeed), S(LoadFreezeFile), S(LoadMovie),     \
      S(LoadOopsFile), S(Pause), S(QuickLoad000), S(QuickLoad001),            \
      S(QuickLoad002), S(QuickLoad003), S(QuickLoad004), S(QuickLoad005),     \
      S(QuickLoad006), S(QuickLoad007), S(QuickLoad008), S(QuickLoad009),     \
      S(QuickLoad010), S(QuickSave000), S(QuickSave001), S(QuickSave002),     \
      S(QuickSave003), S(QuickSave004), S(QuickSave005), S(QuickSave006),     \
      S(QuickSave007), S(QuickSave008), S(QuickSave009), S(QuickSave010),     \
      S(Reset), S(SaveFreezeFile), S(SaveSPC), S(Screenshot), S(SeekToFrame), \
      S(SoftReset), S(SoundChannel0), S(SoundChannel1), S(SoundChannel2),     \
      S(SoundChannel3), S(SoundChannel4), S(SoundChannel5), S(SoundChannel6), \
      S(SoundChannel7), S(SoundChannelsOn), S(SwapJoypads), S(ToggleBG0),     \
      S(ToggleBG1), S(ToggleBG2), S(ToggleBG3), S(ToggleEmuTurbo),            \
      S(ToggleSprites), S(ToggleTransparency)

#define S(x) x

enum command_numbers { THE_COMMANDS, LAST_COMMAND };

#undef S
#define S(x) #x

static const char* command_names[LAST_COMMAND + 1] = {THE_COMMANDS, NULL};
...
void S9xControlsSoftReset(void) {
  for (set<struct exemulti*>::iterator it = exemultis.begin();
       it != exemultis.end(); it++)
    delete *it;
  exemultis.clear();
...
void S9xUnmapAllControls(void) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (!(mouse[i].crosshair.set & 1))
      mouse[i].crosshair.img =
          0;  // no image for mouse because its only logical position is
...
void S9xSetController(int port,
  ...
  switch (controller) {
    ...
    case CTL_MOUSE:
      ...
      if (!Settings.MouseMaster) {
        S9xMessage(S9X_CONFIG_INFO, S9X_ERROR,
                   "Cannot select SNES Mouse: MouseMaster disabled");
        break;
      }
    ...
    case CTL_JUSTIFIER:
      ...
      if (!Settings.JustifierMaster) {
        S9xMessage(S9X_CONFIG_INFO, S9X_ERROR,
                   "Cannot select Konami Justifier: JustifierMaster disabled");
        break;
      }
  ...
  for (port = 0; port < 2; port++) {
    switch (i = newcontrollers[port]) {
      ...
      case MOUSE1:
        ...
        if (used[i]++ > 0) {
          snprintf(buf, sizeof(buf),
                   "Mouse%d used more than once! Disabling extra instances",
                   i - MOUSE0 + 1);
      ...
      case SUPERSCOPE:
        ...
        if (used[i]++ > 0) {
          snprintf(buf, sizeof(buf),
                   "Superscope used more than once! Disabling extra instances");
          S9xMessage(S9X_CONFIG_INFO, S9X_ERROR, buf);
          newcontrollers[port] = NONE;
          ret = true;
          break;
      ...
      case TWO_JUSTIFIERS:
        ...
        if (used[ONE_JUSTIFIER]++ > 0) {
          snprintf(buf, sizeof(buf),
                   "Justifier used more than once! Disabling extra instances");
          S9xMessage(S9X_CONFIG_INFO, S9X_ERROR, buf);
          newcontrollers[port] = NONE;
          ret = true;
          break;
        ...
        for (i = 0; i < 4; i++) {
          if (mp5[port].pads[i] != NONE) {
            if (used[mp5[port].pads[i] - JOYPAD0]++ > 0) {
              snprintf(
                  buf, sizeof(buf),
                  "Joypad%d used more than once! Disabling extra instances",
                  mp5[port].pads[i] - JOYPAD0 + 1);
              S9xMessage(S9X_CONFIG_INFO, S9X_ERROR, buf);
...
void S9xGetController(int port,
                      enum controllers* controller,
                      int8* id1,
                      int8* id2,
                      int8* id3,
...
void S9xReportControllers(void) {
  ...
  for (int port = 0; port < 2; port++) {
    ...
    switch (newcontrollers[port]) {
      ...
      case MOUSE1:
        c +=
            sprintf(c, "Mouse #%d. ", (int)(newcontrollers[port] - MOUSE0 + 1));
        break;
...
char* S9xGetCommandName(s9xcommand_t command) {
  ...
  switch (command.type) {
    ...
    case S9xButtonSuperscope:
      if (!command.button.scope.fire && !command.button.scope.cursor &&
          !command.button.scope.turbo && !command.button.scope.pause &&
          !command.button.scope.aim_offscreen)
        return (strdup("None"));
    ...
    case S9xButtonJustifier:
      if (!command.button.justifier.trigger &&
          !command.button.justifier.start &&
          !command.button.justifier.aim_offscreen)
        return (strdup("None"));
    ...
    case S9xPointer:
      if (!command.pointer.aim_mouse0 && !command.pointer.aim_mouse1 &&
          !command.pointer.aim_scope && !command.pointer.aim_justifier0 &&
          !command.pointer.aim_justifier1)
        return (strdup("None"));
    ...
    case S9xPointerPort:
      return (
          strdup("BUG: Port should have handled this instead of calling "
                 "S9xGetCommandName()"));
    ...
    case S9xButtonMulti: {
      ...
      for (s9xcommand_t* m = multis[command.button.multi_idx];
           m->multi_press != 3; m++) {
  ...
  if (!strcmp(name, "None"))
    ...
    if (!strncmp(name + 8, "Axis ", 5)) {
      ...
      if (cmd.button.joypad.toggle &&
          !(cmd.button.joypad.sticky || cmd.button.joypad.turbo))
        return (cmd);
    ...
    if ((cmd.button.scope.pause = strncmp(s, "Pause", 5) ? 0 : 1)) {
      s += i = 5;
    }
    ...
    if ((cmd.button.justifier.start = strncmp(s, "Start", 5) ? 0 : 1)) {
      s += i = 5;
    }
...
bool S9xMapButton(uint32 id, s9xcommand_t mapping, bool poll) {
  ...
  if (poll) {
    if (id >= PseudoButtonBase)
      fprintf(stderr,
              "INFO: Ignoring attempt to set pseudo-button #%d to polling\n",
              id - PseudoButtonBase);
    else {
      switch (mapping.type) {
        case S9xButtonJoypad:
...
void S9xReportButton(uint32 id, bool pressed) {
  ...
  if (keymap[id].type ==
      S9xButtonCommand)  // skips the "already-pressed check" unless it's a
                         // command, as a hack to work around the following
                         // problem:
    if (keymap[id].button_norpt ==
        pressed)  // FIXME: this makes the controls "stick" after loading a
                  // savestate while recording a movie and holding any button
      return;
...
bool S9xMapPointer(uint32 id, s9xcommand_t mapping, bool poll) {
  ...
  if (mapping.type == S9xPointer) {
    if (mapping.pointer.aim_mouse0 && mouse[0].ID != InvalidControlID &&
        mouse[0].ID != id) {
      fprintf(stderr,
              "ERROR: Rejecting attempt to control Mouse1 with two pointers\n");
      return (false);
    }

    if (mapping.pointer.aim_mouse1 && mouse[1].ID != InvalidControlID &&
        mouse[1].ID != id) {
      fprintf(stderr,
              "ERROR: Rejecting attempt to control Mouse2 with two pointers\n");
      return (false);
    }

    if (mapping.pointer.aim_scope && superscope.ID != InvalidControlID &&
        superscope.ID != id) {
      fprintf(
          stderr,
          "ERROR: Rejecting attempt to control SuperScope with two pointers\n");
      return (false);
    }

    if (mapping.pointer.aim_justifier0 && justifier.ID[0] != InvalidControlID &&
        justifier.ID[0] != id) {
      fprintf(
          stderr,
          "ERROR: Rejecting attempt to control Justifier1 with two pointers\n");
      return (false);
    }

    if (mapping.pointer.aim_justifier1 && justifier.ID[1] != InvalidControlID &&
        justifier.ID[1] != id) {
      fprintf(
          stderr,
          "ERROR: Rejecting attempt to control Justifier2 with two pointers\n");
      return (false);
    }
  }
  ...
  if (poll) {
    if (id >= PseudoPointerBase)
      fprintf(stderr,
              "INFO: Ignoring attempt to set pseudo-pointer #%d to polling\n",
              id - PseudoPointerBase);
    else {
      switch (mapping.type) {
        case S9xPointer:
  ...
  switch (cmd.type) {
    ...
    case S9xButtonJoypad:
      if (cmd.button.joypad.toggle) {
        ...
        st = r & joypad[cmd.button.joypad.idx].togglestick &
             joypad[cmd.button.joypad.idx].toggleturbo;
        r ^= st;
        ...
        if (data1) {
          if (!Settings.UpAndDown &&
              !S9xMoviePlaying())  // if up+down isn't allowed AND we are NOT
                                   // playing a movie,
          {
            if (cmd.button.joypad.buttons &
                (SNES_LEFT_MASK | SNES_RIGHT_MASK)) {
              // if we're pressing left or right, then unpress and unturbo them
              // both first so we don't end up hittnig left AND right
              // accidentally. Note though that the user can still do it on
              // purpose, if Settings.UpAndDown = true. This is a feature, look
              // up glitches in tLoZ:aLttP to find out why.
              joypad[cmd.button.joypad.idx].buttons &=
                  ~(SNES_LEFT_MASK | SNES_RIGHT_MASK);
              joypad[cmd.button.joypad.idx].turbos &=
                  ~(SNES_LEFT_MASK | SNES_RIGHT_MASK);
            }
      ...
      if (data1) {
        ...
        if (cmd.button.scope.turbo) {
          ...
          if (superscope.phys_buttons & SUPERSCOPE_TURBO)
            superscope.next_buttons |=
                superscope.phys_buttons & (SUPERSCOPE_FIRE | SUPERSCOPE_CURSOR);
          else
            superscope.next_buttons &= ~(SUPERSCOPE_FIRE | SUPERSCOPE_CURSOR);
        }

        superscope.next_buttons |=
            i & (SUPERSCOPE_FIRE | SUPERSCOPE_CURSOR | SUPERSCOPE_PAUSE);

        if (!S9xMovieActive())  // PPU modification during non-recordable
                                // command screws up movie synchronization
          if ((superscope.next_buttons &
               (SUPERSCOPE_FIRE | SUPERSCOPE_CURSOR)) &&
      ...
      if (!data1) {
        ...
        switch ((enum command_numbers)(i = cmd.button.command)) {
          ...
          case ClipWindows:
            ...
            DisplayStateChange("Graphic clip windows",
          ...
          case LoadOopsFile: {
            ...
            char drive[_MAX_DRIVE + 1], dir[_MAX_DIR + 1], def[_MAX_FNAME + 1],
                ext[_MAX_EXT + 1];

            _splitpath(Memory.ROMFilename, drive, dir, def, ext);
            snprintf(filename, PATH_MAX + 1, "%s%s%s.%.*s",
          ...
          case QuickLoad010: {
            ...
            _splitpath(Memory.ROMFilename, drive, dir, def, ext);
            snprintf(filename, PATH_MAX + 1, "%s%s%s.%03d",
                     S9xGetDirectory(SNAPSHOT_DIR), SLASH_STR, def,
                     i - QuickLoad000);
          ...
          case BeginRecordingMovie:
            if (S9xMovieActive())
              S9xMovieStop(FALSE);
            S9xMovieCreate(S9xChooseMovieFilename(FALSE), 0xFF,
                           MOVIE_OPT_FROM_RESET, NULL, 0);
            break;
          ...
          case SwapJoypads:
            if ((curcontrollers[0] != NONE &&
                 !(curcontrollers[0] >= JOYPAD0 &&
                   curcontrollers[0] <= JOYPAD7))) {
              S9xSetInfoString("Cannot swap pads: port 1 is not a joypad");
              break;
            }

            if ((curcontrollers[1] != NONE &&
                 !(curcontrollers[1] >= JOYPAD0 &&
                   curcontrollers[1] <= JOYPAD7))) {
          ...
          case SeekToFrame:
            if (S9xMovieActive()) {
              sprintf(buf, "Select frame number (current: %d)",
        ...
        if (cmd.button.pointer.UD) {
          if (!pseudopointer[cmd.button.pointer.idx].V_adj)
            pseudopointer[cmd.button.pointer.idx].V_adj =
                cmd.button.pointer.UD *
                ptrspeeds[cmd.button.pointer.speed_type];
          pseudopointer[cmd.button.pointer.idx].V_var =
              (cmd.button.pointer.speed_type == 0);
        }

        if (cmd.button.pointer.LR) {
          if (!pseudopointer[cmd.button.pointer.idx].H_adj)
            pseudopointer[cmd.button.pointer.idx].H_adj =
                cmd.button.pointer.LR *
                ptrspeeds[cmd.button.pointer.speed_type];
          pseudopointer[cmd.button.pointer.idx].H_var =
              (cmd.button.pointer.speed_type == 0);
        }
      } else {
        if (cmd.button.pointer.UD) {
        ...
        if (cmd.axis.pointer.HV) {
          if (!pseudopointer[cmd.axis.pointer.idx].V_adj)
            pseudopointer[cmd.axis.pointer.idx].V_adj =
                (int16)((int32)data1 * ptrspeeds[cmd.axis.pointer.speed_type] /
                        32767);
          pseudopointer[cmd.axis.pointer.idx].V_var =
              (cmd.axis.pointer.speed_type == 0);
        } else {
          if (!pseudopointer[cmd.axis.pointer.idx].H_adj)
            pseudopointer[cmd.axis.pointer.idx].H_adj =
                (int16)((int32)data1 * ptrspeeds[cmd.axis.pointer.speed_type] /
                        32767);
          pseudopointer[cmd.axis.pointer.idx].H_var =
              (cmd.axis.pointer.speed_type == 0);
        }
      }
    ...
    switch (i = curcontrollers[n]) {
      case MP5:
        ...
        for (i = 0; i < 2; i++, j++) {
          ...
          if (r >= 16)
            ...
            bits |=
                ((joypad[mp5[n].pads[j] - JOYPAD0].buttons & (0x8000 >> r)) ? 1
                                                                            : 0)
                << i;
        }
      ...
      case JOYPAD7:
        if (read_idx[n][0] >= 16) {
          ...
          return (bits |
                  ((joypad[i - JOYPAD0].buttons & (0x8000 >> read_idx[n][0]++))
                       ? 1
                       : 0));
      ...
      case MOUSE1:
        if (read_idx[n][0] < 8) {
          ...
          return (bits |
                  ((mouse[i - MOUSE0].buttons & (0x8000 >> read_idx[n][0]++))
                       ? 1
                       : 0));
        else if (read_idx[n][0] < 24)
          return (bits |
                  ((mouse[i - MOUSE0].delta_y & (0x800000 >> read_idx[n][0]++))
                       ? 1
                       : 0));
        else if (read_idx[n][0] < 32)
          return (bits | ((mouse[i - MOUSE0].delta_x &
                           (0x80000000 >> read_idx[n][0]++))
                              ? 1
                              : 0));
        else {
          read_idx[n][0]++;
      ...
      case SUPERSCOPE:
        if (read_idx[n][0] < 8)
          return (
              bits |
              ((superscope.read_buttons & (0x80 >> read_idx[n][0]++)) ? 1 : 0));
        else {
          read_idx[n][0]++;
          return (bits | 1);
        }
      ...
      case TWO_JUSTIFIERS:
        if (read_idx[n][0] < 24)
          ...
          return (
              bits |
              ((justifier.buttons & (0x80000000 >> read_idx[n][0]++)) ? 1 : 0));
        else {
          read_idx[n][0]++;
          return (bits | 1);
        }
...
void S9xDoAutoJoypad(void) {
  ...
  for (int n = 0; n < 2; n++) {
    switch (i = curcontrollers[n]) {
      case MP5:
        ...
        for (i = 0; i < 2; i++, j++) {
          if (mp5[n].pads[j] == NONE)
            ...
            WRITE_WORD(Memory.FillRAM + 0x4218 + n * 2 + i * 4,
                       joypad[mp5[n].pads[j] - JOYPAD0].buttons);
        }
      ...
      case JOYPAD7:
        read_idx[n][0] = 16;
        WRITE_WORD(Memory.FillRAM + 0x4218 + n * 2,
                   joypad[i - JOYPAD0].buttons);
...
void S9xControlEOF(void) {
  ...
  for (int n = 0; n < 8; n++) {
    ...
    S9xReportPointer(PseudoPointerBase + n, pseudopointer[n].x,
                     pseudopointer[n].y);
  }
...
void S9xSetControllerCrosshair(enum crosscontrols ctl,
                               int8 idx,
                               const char* fg,
                               const char* bg) {
  struct crosshair* c;
  ...
  switch (ctl) {
    ...
    default:
      fprintf(stderr,
              "S9xSetControllerCrosshair() called with an invalid controller "
              "ID %d\n",
              ctl);
      return;
  }
  ...
  if (fg) {
    ...
    if (i > 15 || fgcolor == 16) {
      fprintf(stderr,
              "S9xSetControllerCrosshair() called with invalid fgcolor\n");
      return;
    }
  }
  ...
  if (bg) {
    ...
    if (i > 15 || bgcolor == 16) {
      fprintf(stderr,
              "S9xSetControllerCrosshair() called with invalid bgcolor\n");
      return;
    }
  }
...
void S9xGetControllerCrosshair(enum crosscontrols ctl,
                               int8* idx,
                               const char** fg,
                               const char** bg) {
  struct crosshair* c;
  ...
  switch (ctl) {
    ...
    default:
      fprintf(stderr,
              "S9xGetControllerCrosshair() called with an invalid controller "
              "ID %d\n",
              ctl);
      return;
  }
...
void S9xControlPostLoadState(struct SControlSnapshot* s) {
  if (curcontrollers[0] == MP5 && s->ver < 1) {
    ...
    S9xMessage(S9X_WARNING, S9X_FREEZE_FILE_INFO,
...
bool MovieGetMouse(int i, uint8 out[5]) {
  if (i < 0 || i > 1 ||
      (curcontrollers[i] != MOUSE0 && curcontrollers[i] != MOUSE1))
    return (false);
...
bool MovieGetJustifier(int i, uint8 out[11]) {
  if (i < 0 || i > 1 ||
      (curcontrollers[i] != ONE_JUSTIFIER &&
       curcontrollers[i] != TWO_JUSTIFIERS))
    return (false);
