...
bool8 S9xMapInput(const char* n, s9xcommand_t* cmd) {
  ...
  if (!strncmp(n, "PseudoButton", 12)) {
    if (isdigit(n[12]) && (j = strtol(n + 12, &c, 10)) < 256 &&
        (c == NULL || *c == '\0')) {
...
void S9xSetupDefaultKeymap(void) {
  ...
  for (ConfigFile::secvec_t::iterator i = keymaps.begin(); i != keymaps.end();
       i++) {
...
void S9xParseInputConfig(ConfigFile& conf, int pass) {
  ...
  if (!conf.GetBool("Unix::ClearAllControls", false)) {
    /*
        // Using 'Joypad# Axis'
        keymaps.push_back(strpair_t("J00:Axis0", "Joypad1 Axis Left/Right
       T=50%")); keymaps.push_back(strpair_t("J00:Axis1", "Joypad1 Axis Up/Down
       T=50%"));
    ...
    */
    ...
    keymaps.push_back(strpair_t("K00:SDLK_RSHIFT", "Joypad1 Select"));
    keymaps.push_back(strpair_t("K00:SDLK_z", "Joypad1 A"));
    keymaps.push_back(strpair_t("K00:SDLK_x", "Joypad1 B"));
    keymaps.push_back(strpair_t("K00:SDLK_a", "Joypad1 X"));
    keymaps.push_back(strpair_t("K00:SDLK_s", "Joypad1 Y"));
    keymaps.push_back(strpair_t("K00:SDLK_d", "Joypad1 L"));
    keymaps.push_back(strpair_t("K00:SDLK_c", "Joypad1 R"));
    ...
    name_sdlkeysym["SDLK_RSHIFT"] = SDLK_RSHIFT;
  }
...
void S9xInitInputDevices(void) {
   ...
   * domaemon: 3) relate paddev1 to SDL_Joystick[0], paddev2 to
   * SDL_Joystick[1]... domaemon: 4) print out the joystick name and
   * capabilities
   */
  ...
  if (num_joysticks == 0) {
    ...
    for (int i = 0; i < num_joysticks; i++) {
      ...
      printf("  %s\n", SDL_JoystickName(i));
      printf(
          "  %d-axis %d-buttons %d-balls %d-hats \n",
          SDL_JoystickNumAxes(joystick[i]), SDL_JoystickNumButtons(joystick[i]),
          SDL_JoystickNumBalls(joystick[i]), SDL_JoystickNumHats(joystick[i]));
    }
  }
}
...
void S9xProcessEvents(bool8 block) {
  ...
  while ((block) || (SDL_PollEvent(&event) != 0)) {
    switch (event.type) {
      case SDL_KEYDOWN:
      case SDL_KEYUP:
        // domaemon: not sure it's the best idea, but reserving the SDLK_q for
        // quit.
        if (event.key.keysym.sym == SDLK_ESCAPE) {
          quit_state = TRUE;
        } else {
          /*          S9xReportButton(event.key.keysym.mod << 16 |  // keyboard
             mod event.key.keysym.sym,     // keyboard ksym event.type ==
             SDL_KEYDOWN);   // press or release
                                                            */
          S9xReportButton(event.key.keysym.sym,        // keyboard ksym
