...
static const char *s9x_base_dir = NULL, *rom_filename = NULL,
                  *snapshot_filename = NULL, *play_smv_filename = NULL,
...
static const char dirNames[13][32] = {"",            // DEFAULT_DIR
                                      "",            // HOME_DIR
                                      "",            // ROMFILENAME_DIR
                                      "rom",         // ROM_DIR
                                      "sram",        // SRAM_DIR
...
void S9xExtraUsage(void)  // domaemon: ExtraUsage -> ExtraDisplayUsage
{
  ...
  S9xMessage(S9X_INFO, S9X_USAGE,
             "-multi                          Enable multi cartridge system");
  S9xMessage(S9X_INFO, S9X_USAGE,
             "-carta <filename>               ROM in slot A (use with -multi)");
  S9xMessage(S9X_INFO, S9X_USAGE,
             "-cartb <filename>               ROM in slot B (use with -multi)");
  S9xMessage(S9X_INFO, S9X_USAGE, "");

  S9xMessage(S9X_INFO, S9X_USAGE,
             "-buffersize                     Sound generating buffer size in "
             "millisecond");
  S9xMessage(S9X_INFO, S9X_USAGE, "");

  S9xMessage(S9X_INFO, S9X_USAGE,
             "-loadsnapshot                   Load snapshot file at start");
  S9xMessage(
      S9X_INFO, S9X_USAGE,
      "-playmovie <filename>           Start emulator playing the .smv file");
  S9xMessage(
      S9X_INFO, S9X_USAGE,
      "-recordmovie <filename>         Start emulator recording the .smv file");
  S9xMessage(S9X_INFO, S9X_USAGE,
             "-dumpstreams                    Save audio/video data to disk");
  S9xMessage(S9X_INFO, S9X_USAGE,
             "-dumpmaxframes <num>            Stop emulator after saving "
             "specified number of");
  S9xMessage(S9X_INFO, S9X_USAGE,
             "                                frames (use with -dumpstreams)");
  S9xMessage(S9X_INFO, S9X_USAGE, "");
  ...
  if (!strncmp((const char*)Memory.NSRTHeader + 24, "NSRT", 4)) {
    ...
    switch (Memory.NSRTHeader[29]) {
      ...
      case 0x24:  // Pad or Mouse in Port 0, Pad or Super Scope in Port 1
        // There should be a toggles here for what to put in, I'm leaving it at
        // gamepad for now
        break;

      case 0x27:  // Pad or Mouse in Port 0, Pad or Mouse or Super Scope in Port
                  // 1
        // There should be a toggles here for what to put in, I'm leaving it at
        // gamepad for now
        break;
...
const char* S9xGetDirectory(enum s9x_getdirtype dirtype) {
  ...
  if (dirNames[dirtype][0])
    snprintf(s, PATH_MAX + 1, "%s%s%s", s9x_base_dir, SLASH_STR,
             dirNames[dirtype]);
  else {
    switch (dirtype) {
      case DEFAULT_DIR:
...
const char* S9xSelectFilename(const char* def,
                              const char* dir1,
                              const char* ext1,
                              const char* title) {
  static char s[PATH_MAX + 1];
...
bool8 S9xOpenSnapshotFile(const char* filename, bool8 read_only, STREAM* file) {
  ...
  if (*drive || *dir == SLASH_CHAR ||
      (strlen(dir) > 1 && *dir == '.' && *(dir + 1) == SLASH_CHAR)) {
    strncpy(s, filename, PATH_MAX + 1);
    s[PATH_MAX] = 0;
  } else
    snprintf(s, PATH_MAX + 1, "%s%s%s", S9xGetDirectory(SNAPSHOT_DIR),
             SLASH_STR, fname);
...
void S9xSyncSpeed(void) {
  ...
  // If we're on AUTO_FRAMERATE, we'll display frames always only if there's
  // excess time. Otherwise we'll display the defined amount of frames.
  ...
  if (IPPU.RenderThisFrame)
    ...
    if (timercmp(&next1, &now, <)) {
      unsigned lag =
          (now.tv_sec - next1.tv_sec) * 1000000 + now.tv_usec - next1.tv_usec;
      if (lag >= 500000) {
  ...
  // Can't use setitimer because the sound code already could be using it. We
  // don't actually need it either.
  while (timercmp(&next1, &now, >)) {
    ...
    while (gettimeofday(&now, NULL) == -1)
      ;
    // Continue with a while-loop because usleep() could be interrupted by a
    // signal.
  }
...
int main(int argc, char** argv) {
  ...
  if (!Memory.Init() || !S9xInitAPU()) {
    fprintf(stderr,
            "Snes9x: Memory allocation failure - not enough RAM/virtual memory "
            "available.\nExiting...\n");
    Memory.Deinit();
    S9xDeinitAPU();
    exit(1);
  ...
  if (Settings.Multi) {
    ...
    if (!loaded) {
      ...
      if (Settings.CartAName[0]) {
        ...
        snprintf(s1, PATH_MAX + 1, "%s%s%s", S9xGetDirectory(ROM_DIR),
                 SLASH_STR, fname);
      ...
      if (Settings.CartBName[0]) {
        ...
        snprintf(s2, PATH_MAX + 1, "%s%s%s", S9xGetDirectory(ROM_DIR),
                 SLASH_STR, fname);
    ...
    if (!loaded && rom_filename[0]) {
      ...
      snprintf(s, PATH_MAX + 1, "%s%s%s", S9xGetDirectory(ROM_DIR), SLASH_STR,
               fname);
  ...
  if (Settings.NetPlay) {
    ...
    fprintf(stderr,
            "Connected to server %s on port %d as player #%d playing %s.\n",
  ...
  if (play_smv_filename) {
    ...
    uint32 flags = CPU.Flags & (DEBUG_MODE_FLAG | TRACE_FLAG);
    if (S9xMovieCreate(record_smv_filename, 0xFF, MOVIE_OPT_FROM_RESET, NULL,
                       0) != SUCCESS)
      exit(1);
    CPU.Flags |= flags;
